# Changelog

이 파일은 **0.8.0부터** 기록한다. 그 이전 릴리스의 **내용**은 git 태그(`v0.3.0` ~ `v0.7.0`)와
커밋 이력을 참조할 것 — 사후에 재구성해 적으면 실제와 어긋날 수 있어 소급하지 않았다.

**단 한 가지는 소급 기재한다 — 생성기가 해석하는 attribute 가 「어느 버전부터 쓸 수
있는가」**(맨 아래 `0.5.0`·`0.3.0` 두 절). 재구성이 아니라 **기계 도출**이라 위 우려가
성립하지 않는다 — 각 attribute 를 처음 소비한 커밋을 `git log -S` 로 특정하고, 그 커밋을
포함하는 최초 태그를 `git tag --contains` 로 뽑았다. 이 다섯은 전부 기록 시작점 이전
산물이라, 그 전까지는 **릴리스 이력만 읽어서는 존재 자체를 알 수 없었다.**

버전 정책: `0.MINOR.PATCH`. MINOR는 기능 추가, PATCH는 수정.

**여러 버전을 건너뛰어 올린다면 그 사이의 모든 항목을 읽을 것.** 각 릴리스는 자기 breaking만
적으므로, 최신 항목 하나만 읽으면 건너뛴 구간의 동작 변화가 그대로 남는다.

---

## [Unreleased]

### 🔴 breaking — `@reference` 뒤의 기호가 삭제 동작을 낸다

`@reference(X)` 뒤의 `!`·`?`·`!!`(M3L §8.4.1)는 지금까지 **읽히지 않아** 어떤 FK 에도 `ON DELETE` 가 붙지 않았다(DB 기본값
NO ACTION). 이제 두 SQL 방언이 쓴다:

| 기호 | SQL Server | PostgreSQL |
|---|---|---|
| `!` | `ON DELETE NO ACTION` | `ON DELETE NO ACTION` |
| `?` | `ON DELETE SET NULL` | `ON DELETE SET NULL` |
| `!!` | `ON DELETE NO ACTION`(SQL Server 에는 RESTRICT 가 없고 NO ACTION 을 즉시 검사한다) | `ON DELETE RESTRICT` |

- **`?` 를 쓴 모델은 삭제 동작이 바뀐다** — 참조된 행을 지우면 이제 이 키가 NULL 이 된다(전에는 삭제가 거절됐다). 재생성 후 DDL 의
  차이를 확인할 것.
- 🆕 **SQL Server 방언은 SQL Server 가 거절할 `?` 조합에서 빌드를 세운다**(오류 1785 — 한 삭제가 같은 테이블에 두 번 닿거나 자기
  테이블로 돌아오는 연쇄). `?` 자기참조 · 한 모델에서 다른 모델로 가는 `?` 키 둘 · `::aspect` 키(CASCADE)와 같은 쌍의 `?` 가 그렇다.
  배포 도중이 아니라 생성 시점에 멈추고, 메시지가 그 경로를 적는다 — 그중 하나를 `!` 로 두고 NULL 처리는 애플리케이션에서.
  SET NULL 은 갱신한 행에서 끝나므로 서로 다른 모델의 `?` 가 고리를 이루거나 한 테이블에서 만나는 모델은 그대로 된다.
  PostgreSQL 방언은 이 제한이 없어 그대로 쓴다.
- 🆕 **MDD023** — `?` 인데 키가 NULL 을 허용하지 않으면 빌드가 선다. DB 가 그 제약을 거절하기 때문이다(SQL Server 는 생성 시,
  PostgreSQL 은 첫 삭제 시). 필드를 `identifier?` 로 하거나 다른 기호를 쓴다.
- **기호 없는 `@reference` 는 그대로다** — 절을 쓰지 않는다. 명세의 «자동 결정»(nullable → SET NULL, 아니면 CASCADE)은 적용하지
  않는다: SQL Server 에서 자기참조나 같은 모델을 가리키는 키 둘은 둘 다 연쇄할 수 없어(오류 1785) 흔한 모델이 배포되지 않게 된다.
- **Model 타깃(EF Core)도 같은 동작을 구성한다** — 쓰기 엔티티의 navigation 에 `OnDelete(DeleteBehavior.NoAction | SetNull | Restrict)`.
  종전에는 EF 관례(필수 키 = Cascade)가 추적 중인 자식 행을 지웠다 — DB 가 거절하라고 들은 삭제를 클라이언트가 먼저 수행한 셈이다. 기호 없는
  키는 관례 그대로.

라이브러리로 렌더러를 부르는 경우: `ColumnRenderer.Render` 의 `bool cascadeOnDelete` 가 `ReferentialAction? onDelete` 로 바뀌었다
(`MddBooster.Core.Semantic.OnDeleteRule.For` 가 모델·필드에서 그 값을 준다).

### 파서 `M3L.Native` 0.15.0

- **lookup 경로의 조각이 그 자리의 모델에 없는 필드이면** 이제 파서 검증기가 `M3L-E022` 로 먼저 세운다 — 첫 hop 의 키,
  둘째 hop 부터의 키, 경로 끝 필드 모두. 종전에도 같은 모델은 이 생성기의 `MDD004`(키)·`MDD006`(끝 필드)로 멈췄으므로
  **빌드되던 모델이 멈추는 일은 없다** — 바뀌는 것은 메시지의 층(`[m3l]` 대 `[semantic]`)과 코드뿐이다. 종료 코드는 둘 다 3.
  빌드 로그에서 `MDD004`·`MDD006` 을 찾던 곳이 있다면 `M3L-E022` 도 함께 볼 것.
- 파서가 내는 AST 의 맵 키(`sections.metadata`·사용자 정의 섹션·`extensions`) 순서가 실행·플랫폼마다 같아졌다. 이 생성기의
  출력은 바뀌지 않는다(같은 모델의 재생성 결과가 바이트 단위로 같다).

### 추가 — 표면 타깃을 소유자(`# Prefix:`)로 고르는 `includeOwners` / `excludeOwners`

한 정본을 베이스 파일과 모듈 파일로 나눠 두고 표면(Api·TypeScript)을 모듈 단위로 가르려면, 지금까지는 같은 엔티티
이름을 한 타깃의 `excludeEntities` 와 다른 타깃의 `includeEntities` 에 두 번 적어야 했고, 모듈에 모델이 늘면 모듈
타깃에서는 조용히 빠지고 베이스 타깃에는 조용히 나타났다.

- 이제 모델을 선언한 파일의 `# Prefix:` 로 고른다. 접두어 없는 파일은 `""`. 모델이 늘어도 설정이 바뀌지 않는다.
- 엔티티 목록과 같은 규칙(표면 타깃 전용 · 동시 지정 오류 · 없는 소유자 오류)이고, 엔티티 목록과는 교집합.
- TypeScript 타깃에서는 enum 도 같은 축으로 갈린다 — 자기 소유자의 enum 전부 + 다른 소유자의 enum 중 자기 모델이
  참조하는 것만. 소유자 축이 없는 타깃의 출력은 한 바이트도 바뀌지 않는다.
- `sharedTypesImport` — 모듈 타깃이 `IyuEntity` 와 다른 소유자의 enum 을 다시 선언하지 않고 지정한 배럴(베이스 타깃
  출력의 재수출)에서 재수출한다. 두 타깃에 같은 이름의 타입이 두 벌 생기지 않는다. 소유자 축이 있는 TypeScript 타깃
  전용.

### 추가 — 경고를 에러로 올리는 `mdd.json` 키

`mdd build` 는 경고가 있어도 종료 코드 0 이어서, «경고 0 빌드»를 지키려면 콘솔 문구(로캘·형식이 바뀔 수 있는)를 찾아야
했다. MSBuild 와 같은 이름·같은 우선순위의 최상위 키 셋을 더했다:

- `treatWarningsAsErrors: true` — 코드 있는 경고 전부를 에러로. `warningsAsErrors: ["MDD017", …]` — 나열한 코드만.
  `noWarn: ["MDD018", …]` — 보고하지 않음(두 승격 키보다 우선).
- 대상은 이 생성기의 `MDDnnn` 과 m3l 의 `M3L-Wnnn`. 승격된 경고가 있으면 두 층을 다 보고한 뒤 생성 전에 종료 코드 3.
- 코드 형식이 아닌 값 · 두 목록에 함께 있는 코드는 설정 오류(종료 코드 4).
- 키를 쓰지 않으면 동작은 그대로다.
- `schemas/mdd.schema.json` 최상위에도 세 키가 있다(코드 형식 `pattern` 포함).

### 수정 — 더 이상 내지 않는 생성 폼이 `formsOutputPath` 에 남았다

엔티티를 개명·삭제하거나 타깃 범위(`include/excludeEntities`·`forms…Entities`)에서 빼면, 다른 타깃의 출력(`Entity_gen/`·
`tables_gen/` 등)에서는 옛 파일이 사라지는데 **폼 디렉터리의 옛 `{Entity}Form_gen.tsx` 는 남았다**. 생성물 표시를 단 채
어느 빌드도 내지 않는 파일이라, 옛 타입을 import 해 다음 `tsc` 에서야 드러나거나 조용히 남았고, 재생성 후 diff 검사도
그 파일을 건드리지 않아 잡지 못했다.

- 이제 빌드마다 폼 디렉터리의 `*Form_gen.tsx` 중 **이번에 내지 않았고 첫 줄이 이 생성기의 헤더인 것**을 지우고, 지운 목록을
  빌드 출력에 싣는다(`[ts] 이번 빌드가 내지 않은 생성 폼 N개 삭제: …`). 헤더 없는 손 파일과 다른 이름의 파일은 건드리지 않는다.
- 🔴 **두 TypeScript 타깃이 같은 `formsOutputPath` 를 쓰면 빌드 오류**(종료 코드 4) — 각 타깃이 상대 타깃의 폼을 «내지 않은
  것»으로 보고 지우게 되기 때문이다. 타깃마다 폼 디렉터리를 나눈다.
- `formsOutputPath` 설정 자체를 지우거나 바꾼 경우의 옛 디렉터리는 생성기가 알 수 없어 그대로 남는다.

### 수정 — T-SQL 이 참조 대상의 키를 `[dbo]`·`[Id]` 로 가정했다

Sql 타깃은 다른 모델의 키를 가리키는 세 자리 — `@reference` 의 FK · Lookup 의 `JOIN … ON` · Rollup 의 상관
조건 — 에서 대상 컬럼을 늘 `[Id]` 로, FK 의 스키마를 늘 `[dbo]` 로 적었다. 그래서:

- 대상의 `@pk` 가 `id` 가 아니면(`code: string(2) @pk` 같은 자연 키) **없는 컬럼을 가리키는 FK·JOIN** 이 나와
  배포가 실패했다.
- `schema` 를 `dbo` 가 아닌 값으로 두면 테이블은 그 스키마에, **FK 는 `[dbo]` 에** 걸렸고, 사후 배포 스크립트
  (`Script.PostDeployment.RefreshViews.sql`)는 `[dbo]` 뷰만 찾아 **빈 채로** 나왔다.

이제 세 자리 모두 대상 모델의 실제 `@pk` 컬럼과 설정된 `schema` 를 쓰고, 새로고침 스크립트는 뷰를
`[schema].[name]` 으로 식별한다(손으로 관리하는 뷰가 다른 스키마에 있어도 된다). PostgreSQL 타깃은 이미 그렇게
하고 있었다.

**키가 `id` 이고 `schema` 가 `dbo` 인 모델의 산출물은 한 바이트도 바뀌지 않는다** — 바뀌는 것은 지금까지
배포할 수 없던 출력뿐이다.

라이브러리로 렌더러를 직접 부르는 경우: `TableRenderer.Render` 에 `allModels` 가 추가됐다. `@reference` 가 있는
모델을 그것 없이 렌더하면 키를 짐작하지 않고 예외를 낸다(`FullViewRenderer.Render` 의 기존 `allModels` 도 이제
한 단계 Lookup 에 필요하다).

---

## 0.32.0

### 🔴 breaking — base 를 밝히지 않은 «키 = FK» 는 빌드 오류(`MDD022`)

`- asset_id: identifier @pk @reference(Asset)` 처럼 **키가 곧 다른 모델의 FK** 인데 모델이 `::aspect` 를 선언하지
않으면 빌드가 선다. 그 모양은 base 한 행에 딸린 선택적 정보일 수도, base 의 한 종류일 수도 있고 둘은 다르게
생성되므로(삭제 연쇄 · navigation) 생성기가 짐작하지 않는다. 안내문이 옮길 형태를 그대로 적는다:

```markdown
## AssetMaintenanceProfile ::aspect(Asset) : Timestampable
- level: integer?          # 키 필드는 지운다 — ::aspect(Asset) 가 만든다
```

**옮기면 DDL 이 한 곳 바뀐다**: base 로의 FK 에 `ON DELETE CASCADE` 가 붙는다(aspect 는 base 행과 한 단위로
존재한다). 칼럼 이름·PK 는 그대로다. 그 밖에 base 쪽에 역방향 navigation 이 생긴다(`Asset.MaintenanceProfile`).

### 다단 lookup — `@lookup(order_id.customer_id.name)`

언어 명세 §4.5.1 이 정의하는 다단 경로를 구현한다. 종전에는 경로를 첫 `.` 에서 잘라 `customer_id.name`
이라는 **이름의 칼럼**을 찾았고, 없으니 `MDD006` 으로 거절했다.

- **뷰(두 방언)**: hop 마다 앞 hop 에서 LEFT JOIN 을 잇는다(별칭 `j_order_id` → `j_order_id__customer_id`).
  마지막 hop 앞의 hop 은 대상의 **테이블**을 JOIN 한다 — 다음 키(저장 칼럼)만 읽으므로 다른 모델의 FullView
  에 의존하지 않는다. 마지막 hop 은 종전 1단 규칙 그대로(읽는 칼럼이 파생이면 대상의 FullView). 같은 접두사를
  쓰는 lookup 들은 JOIN 을 공유한다. **1단 lookup 만 가진 모델의 생성 SQL 은 바이트 단위로 그대로다.**
- **순환 탈출구가 실재한다**: A 가 B 의 파생 칼럼을 롤업하고 B 가 A 의 lookup 칼럼을 1단으로 읽으면 FullView
  순환으로 빌드가 멈춘다(종전 그대로). 이제 같은 값을 저장 키로 따라 읽는 다단 경로는 순환 없이 빌드된다 —
  그리고 순환 오류의 안내문이 그 문법을 예로 든다. 종전 안내문은 이 생성기가 거절하던 형태를 권했다.
- **널 허용 전파(§4.5.4)**: 경로의 **어느** 키든 널 허용이면 C# 속성(`string?`)과 TS 인터페이스(`| null`)가
  널 허용이다. 종전에는 첫 hop 키만 봤다.
- **진단**: `MDD004`·`MDD005` 는 둘째 hop 부터 그 키를 찾은 모델을 지목하고, `MDD006` 은 경로 끝 모델을
  지목한다. 1단 경로의 메시지는 그대로다.
- TS 필드 스키마의 라벨 상속(라벨 없는 lookup 이 대상 필드의 `@label` 을 빌림)이 다단 경로에서도 동작한다.

### 파서 `M3L.Native` 0.14.0 — enum 상속이 생성물에 닿는다

- **`## Child ::enum : Parent` 가 이제 동작한다.** 파서가 부모의 값을 자식의 값 목록에 합쳐 준다(부모 것
  먼저, 조부모가 부모보다 앞, 그다음 자기 블록). C# enum · TypeScript 유니온 · 표시명 맵 · (켜 두었다면)
  SQL CHECK 가 전부 같은 목록을 읽는다. 종전에는 자기 블록의 멤버만 생성됐고, 빌드가 이를
  `::enum X : Base (상속 멤버는 합쳐지지 않는다 …)` 로 알렸다 — **그 알림은 사라졌다.** 멤버 순서가 C#
  서수를 정하지만 저장 값은 멤버 이름이라 이미 저장된 행은 영향이 없다.
- **이 파서의 검증기가 새로 내는 진단**(검증기 에러는 종료 코드 3 — `0.29.0` 부터):
  `M3L-E020` — 상속을 합친 뒤 같은 값 이름이 두 번(한 블록 안의 중복 포함). 종전엔 통과해 생성 C# enum 에 같은 멤버가
  두 번 나갔다. `M3L-E021` — `::extend` 헤더의 괄호 인자. 종전에도 대개 빌드가 섰지만 엉뚱한 대상을 지목한 `M3L-E011`
  로 섰다 — 이제 실제 원인을 말한다.
- **다단 lookup 의 둘째 hop 부터 키에 `@reference` 가 없으면** 이제 파서 검증기가 `M3L-E002` 로 먼저 세운다.
  종전(위 「다단 lookup」)에도 같은 모델은 이 생성기의 `MDD005` 로 멈췄으므로 **빌드되던 모델이 멈추는 일은
  없다** — 바뀌는 것은 메시지의 층(`[m3l]` 대 `[semantic]`)과 코드뿐이다. 종료 코드는 둘 다 3.

### `::aspect` 가 이 생성기에 지우는 규칙 (MDD019 경고 · MDD020·MDD021 오류)

`::aspect(Base)` 는 base 쪽에 navigation 하나를 만든다. 그 이름은 모델명에서 `<Base>` 를 뗀
나머지이고 파일의 `# Prefix:` 는 보존된다 — `AssetMaintenanceProfile` → `Asset.MaintenanceProfile`,
`InspAssetInspectionProfile` → `Asset.InspInspectionProfile`. 이름이 약속을 따르지 않으면 navigation 은
**모델명 전체**가 되고(`Contractor` → `Organization.Contractor`), 그때 **MDD019 경고**가 그 사실을
알린다. 오류가 아닌 이유는 기존 테이블의 개명을 강요하지 않기 위해서다.

한 base 에 같은 navigation 이름을 내는 aspect 가 둘이거나, 그 이름이 base 의 기존 필드와
겹치면 **MDD020 오류**다 — 이쪽은 «덜 좋은 이름»이 아니라 컴파일되지 않는 코드다.

`::aspect` 모델이 **자기 키를 선언하면 MDD021 오류**다. 이 생성기는 aspect 를 「PK 가 곧 base
FK」인 테이블로 내므로 키를 따로 선언할 자리가 없다. ⚠ 이것이 **언어의 규칙이 아니라는 점**이
중요하다 — M3L 명세 §3.4.8 은 `::aspect` 를 정의하면서 *「How a generator stores either is not
part of the language」* 로 저장 방식을 명시적으로 언어 밖에 둔다. 대리 키 + 유니크 FK 로 저장하는
생성기도 그 정의를 만족하고, 그런 생성기에서는 `@pk` 가 모순이 아니다. 모순은 **이 생성기가
PK=FK 를 고른 뒤에** 생긴다.

**base 자체의 규칙은 여기 없다.** base 가 실재하는지, aspect 가 다른 aspect 의 base 가 될 수
있는지는 언어의 규칙이고 M3L 이 이미 거절한다(`M3L-E016`–`E018`). 이 생성기는 그 판정을
되풀이하지 않는다.

### `::aspect` 모델이 실제로 생성된다 — PK 가 곧 base FK 인 테이블

`MDD016` 이 `::aspect` 를 더 이상 막지 않는다. `## AssetMaintenanceProfile ::aspect(Asset)` 는
키를 선언하지 않고, `::aspect(Asset)` 가 만든다 — 물리 컬럼 `AssetId`(T-SQL) / `asset_id`(PG),
그 자체가 `Asset` 으로의 FK, `ON DELETE CASCADE`. aspect 는 base 행과 한 단위로 존재하므로
base 가 사라지면 딸린 행도 사라진다.

**두 방언 모두** 같은 모양을 낸다. 키가 «필드로 합성되기» 때문에 두 렌더러는 aspect 를 특별
취급하지 않고 공유 PK 확장 테이블을 이미 다루던 경로를 그대로 탄다. 보통 `@reference` 의
삭제 동작은 바뀌지 않는다 — cascade 는 `::aspect` 가 만든 키에만 붙는다.

**EF 도 그 관계를 안다.** 생성되는 읽기 타입이 navigation 쌍을 싣고(`AssetMaintenanceProfileExt.Asset`
↔ `Asset.MaintenanceProfile`), `DbContext` 가 `HasOne().WithOne().HasForeignKey<>().OnDelete(Cascade)`
를 **명시 구성**한다. ⚠ 보통 1:1 에는 여전히 구성 코드를 내지 않는다 — 관례가 추론하기 때문이고,
aspect 는 PK 가 곧 FK 라 알아볼 `<Principal>Id` 프로퍼티가 없어 **그 근거가 성립하지 않는다.**
두 처분이 갈리는 이유가 그것이지 예외가 아니다. 두 방언이 같은 구성을 낸다.

⚠ **`::subtype` 은 여전히 `MDD016` 으로 거절된다.** is-a 의 저장 전략(단일 테이블 / 클래스별
테이블 / 구체 클래스별 테이블)이 정해지지 않았고, 하나를 조용히 고르는 것은 이 생성기가 낼
스키마의 모양을 말없이 정하는 일이다.

### 1:1 연관이 읽기 타입에 navigation 쌍을 만든다

`@reference(X)` 와 `@unique` 가 **같은 필드에** 선언되면 그 관계는 1:1 이고, Model 타깃이 이제
읽기(`Ext`) 타입에 navigation 을 **쌍으로** 방출한다 — 의존 쪽에 대상의 `Ext` 를 가리키는
정방향, 대상 쪽에 되돌아오는 역방향. 정방향의 널 허용은 FK 를 따르고 역방향은 언제나 널 허용이다
(대상 행에 의존 행이 없을 수 있다). **쓰기 엔티티는 아무것도 얻지 않는다.**

관계 구성 코드(`HasOne`/`WithOne`)는 방출하지 않는다 — 정방향 이름을 FK 필드명에서 유도하므로
EF 관례가 요구하는 짝이 어긋날 수 없다. 두 방언의 산출물은 엔티티 파일 기준 바이트 단위로 같다.

**기존 소비자에게 이것은 순가산이다.** 재생성하면 읽기 타입에 프로퍼티가 늘어나므로 diff 가
생기지만, 없어지거나 형태가 바뀌는 것은 없다. 자세한 규칙과 예시는 README 의
「1:1 연관 → 읽기 타입의 navigation 쌍」 절.

⚠️ **N:1 `$expand` 는 이것으로 열리지 않는다.** 방출되는 navigation 은 `@unique` 가 붙은 1:1
선언에 한정된다. `@unique` 없는 일반 `@reference`(N:1)는 여전히 읽기 타입에 navigation 을 얻지
않는다.

### 1:1 역방향 이름이 충돌하면 빌드가 멈춘다 (exit 3)

두 의존 모델이 같은 대상을 가리키면서 같은 역방향 이름을 요구하면 빌드가 **에러로 종료**한다
(exit 3). 경고로 두면 한쪽 navigation 이 조용히 사라지거나 엉뚱한 쪽을 가리키게 된다. 출력이
충돌한 쌍을 이름까지 찍고, 해소는 한쪽의 FK 필드명을 바꾸거나 대상을 나누는 것이다.

곁들여, 발견한 1:1 관계는 충돌 여부와 무관하게 **한 줄씩 전부** 빌드 로그에 찍힌다 — 선언한
관계가 빌드 출력에서 보이지 않는 상태를 만들지 않기 위한 것이다.

---

## 0.31.0

### enum 멤버의 표시명이 C# 쪽에서도 런타임에 읽힌다 — `[Display(Name)]`

모델이 enum 멤버에 선언한 표시명(`- planned: "계획됨"`)이 이제 C# 에서도
어트리뷰트로 나간다. 그동안 이 텍스트는 TypeScript 타깃에서는 런타임 맵
(`enum_labels_gen.ts`)으로, C# 에서는 **XML 문서 주석으로만** 나갔다. 주석은 컴파일되며
사라지므로, 서버가 문서를 그리는 경로(보고서·내보내기·메일 본문)에서는 선언된 표시명으로
돌아갈 방법이 없어 멤버 이름이 그대로 찍혔다 — 같은 개념이 화면에서는 표시명으로,
문서에서는 영문 식별자로 보였다.

```csharp
public enum WorkOrderStatus
{
    /// <summary>계획됨</summary>
    [Display(Name = "계획됨")]
    [EnumMember(Value = "planned")]
    Planned,
    ...
}
```

`[EnumMember]`(wire 값)과 역할이 갈린다. 방출 파일에
`using System.ComponentModel.DataAnnotations;` 가 함께 나간다. 읽는 방법:

```csharp
typeof(T).GetField(value.ToString())?.GetCustomAttribute<DisplayAttribute>()?.Name
    ?? value.ToString()
```

**표시명을 선언하지 않은 멤버에는 어트리뷰트가 붙지 않는다** — 필드 라벨과 같은
규칙로, `Name` 이 있다는 것은 사람이 써 넣은 텍스트라는 뜻이지 멤버 이름의 재진술이
아니기 때문이다. 위 폴백이 그 자리를 덤는다. (TypeScript 라벨 맵은 반대 기본값을 쓰고
그래야 한다 — `Record<T, string>` 은 전사여야 한다.)

멤버 아래에 들여쓴 블록인용을 둔 경우, 그 텍스트는 표시명에 섞이지 않는다 —
어트리뷰트는 인라인 라벨만 나른다(필드 쪽과 다르다).

---

## 0.30.0

### 모델 확장(::extend)과 소유자 접두어

M3L 이 `# Prefix:`(파일 소유자)와 `## Target ::extend`(다른 파일이 정의한 모델에 필드를 더하는
블록)를 지원하게 되면서, 이 생성기도 확장 필드를 소비한다. 확장 필드는 SQL 컬럼·C# 속성·
TypeScript 타입 멤버 어디서나 그 모델의 다른 필드와 똑같은 보통 필드로 방출된다 — 타깃별
특수 처리는 없다.

새 의미 분석 규칙 다섯 가지(README 「의미 분석」 표):
- `MDD013` 확장 필드가 대상 모델과 다른 소유자에 속하는데 확장자의 `<prefix>_` 로 시작하지 않음
- `MDD014` 접두어 없는 파일이 접두어 소유 모델을 확장
- `MDD015` 확장 저장 필드가 nullable 도 기본값도 아님
- `MDD016` 베이스를 갖는 모델(`::aspect`/`::subtype`) — 아직 지원하지 않음, 빌드를 세운다
- `MDD017`(경고) 접두어 파일의 모델명이 그 접두어로 시작하지 않음

### ⚠ 속성 오타 경고의 코드가 `MDD006` → `MDD018` 로 바뀌었다

`MDD006` 은 서로 무관한 두 조건 — lookup 대상 컬럼 부재(오류)와 속성 이름 오타 의심(경고) — 에 함께
쓰이고 있었다. 코드로 빌드 로그를 거르면 그 결과가 무엇을 뜻하는지 알 수 없었다. 오타 경고를 `MDD018`
로 옮겼고 `MDD006` 은 오류 전용이다. **빌드 로그에서 `MDD006` 경고를 걸러내던 곳이 있다면 `MDD018` 로
바꿀 것.** 메시지 문구·심각도·발화 조건은 그대로다.

확장이 더하는 필드는 `@group` 을 명시하지 않으면 확장자의 접두어를 기본 그룹으로 삼는다 —
Model 타깃의 `[Display(GroupName = ...)]`, TypeScript 필드 스키마의 `group:`, 생성 폼의 섹션
제목이 모두 이 기본값을 따른다.

빌드는 어느 모델이 누구에게 얼마나 확장됐는지 한 줄로 stdout 에 보고한다(병합 순서, 접두어
없는 출처는 `(standard)`):

```
[m3l] 확장: Asset ← insp(2), (standard)(1)
```

### 파서 `M3L.Native` 0.13.0

파서 핀을 `0.13.0` 으로 올렸다. `::extend`/`::aspect`/`::subtype` 은 더 이상 알려지지 않은
`::kind` 가 모이는 generic extension 버킷에 들어가지 않는다 — 파서가 직접 해석해 각각
`extended_by`/`base` 로 모델에 실어 보낸다. 이 파서가 새로 내는 진단(`M3L-E011`~`E018`)은 위
생성기 규칙과 다른 층이다 — 파서는 언어 규칙(대상 모델 존재·필드명 충돌 등)을, 생성기는 이
생성기가 강제하는 네이밍 정책을 검사한다.

## 0.29.0

### 🔴 m3l 검증기 에러가 빌드를 세운다 (종료 코드 3)

`mdd build` 가 파서 경고만이 아니라 **m3l 검증기의 진단**도 보고한다. `sources` 전체를 한 단위로
검증하므로, 한 파일에 등록한 `::attribute` 를 다른 파일에서 잘못 쓴 경우(M3L-W005~W008)도 잡힌다.
경고는 파서 경고와 같은 형식(`[m3l] 경고 [code] file:line:col msg`)으로 stderr 에 나오고 빌드는
계속된다.

검증기 **에러**는 어떤 타깃도 돌기 전에 빌드를 멈추고 종료 코드 3 을 낸다 — 의미 분석 에러와 같은
코드다. stderr 에는 `[m3l] 에러 N건:` 아래에 각 진단이 나온다.

**업그레이드 영향**: 검증기가 거부하는 모델은 지금까지 빌드됐지만 이제 실패한다 — 정의되지 않은 타입,
중복 모델·enum 이름, 모델 안 중복 필드, 해석되지 않는 부모, 여러 네임스페이스에 걸친 모호한 참조,
키에 `@reference` 가 없는 `### Relations` 항목 등.

### `M3L.Native` 0.12.0 — `byte`·`short`·`double` 이 M3L 타입 카탈로그에 들어갔다

이 생성기가 원래 매핑하던 세 숫자 타입이 이제 언어의 타입 카탈로그에 있다. 위 에러 중단이 가능해진
이유다 — 카탈로그에 없던 동안에는 이 셋을 쓰는 모델이 전부 «정의되지 않은 타입» 에러를 받았을 것이다.
같은 릴리스에서 M3L 은 `float` 을 32-bit 로 명시했다. 이 생성기는 이미 `float` 을 32-bit(`REAL`·
`float`)로, `double` 을 64-bit 로 매핑하고 있어 산출물은 바뀌지 않는다.

### `@min`/`@max` 가 Model 타깃의 `[Range]` 로 방출된다

종전에는 TypeScript 필드 스키마에만 닿았다. 숫자 타입 한정이며, 한쪽만 선언하면 반대쪽은 그 타입의
한계로 열린다.

## 0.28.0

### `M3L.Native` 0.10.1 — enum 값에 대한 `::attribute` 레지스트리 검증(재적용) + 필드 사이 블록인용 귀속 수정

`0.10.0` 이후 `M3L.Native` 를 곧바로 `0.10.1` 로 올렸다. 두 릴리스는 함께 소비된다: `target: [value]`
를 지원해 `::attribute` 레지스트리가 enum 값에도 M3L-W005~W008 검증을 적용하고(`0.10.0`), 필드
사이에 놓인 들여쓰지 않은 블록인용을 그 필드가 아니라 모델 설명으로 귀속시킨다(`0.10.1`).

### 여러 줄(들여쓴 블록인용) 필드 설명이 생성 코드를 컴파일 불가로 만드는 결함 수정

필드 설명에 개행이 있으면 C# `[Display(Name/GroupName/Description)]`, `[Lookup]`/`[Rollup]`/
`[Computed]` 인자, TypeScript `field_schema_gen.ts` 의 `label`/`group`, 생성 폼의 `label`/
`description`/`placeholder`/섹션 제목이 이스케이프되지 않은 생 개행을 문자열 리터럴에 그대로
내보내 C# 컴파일 오류·`tsc` 구문 오류를 냈다. 모든 문자열 리터럴 방출을 언어별 공용 이스케이퍼
하나로 통일해 개행·따옴표·제어문자를 안전하게 처리한다.

**부수 변화(호환성 영향 없음, 재생성 시 diff 발생)**: 생성 폼의 슬롯 문구와 field-override
`label` 리터럴이 `"…"` 에서 `'…'` 로 바뀐다 — 나머지 TypeScript 출력과 같은 따옴표 스타일이다.

## 0.27.0

### `M3L.Native` 0.9.0 — 복합 `@unique`의 NULL 정책 선언(`nulls: "not_distinct"`)

기본적으로 nullable 컬럼이 섞인 복합 `@unique`는 filtered unique index로 방출된다 — NULL이
"미설정"을 뜻할 때는 맞지만, 폴백/오버라이드 테이블처럼 NULL이 "나머지 범위 전체"를 뜻해 그
자체로 유일성 경쟁에 참여해야 하는 경우에는 틀렸다. `nulls: "not_distinct"`(SQL:2023 `UNIQUE
NULLS NOT DISTINCT` 어휘)를 붙이면 그 필터를 만들지 않는다 — SQL Server는 plain `UNIQUE`가
이미 이 의미라 필터 없는 inline `CONSTRAINT UNIQUE`, PostgreSQL 15+는 `UNIQUE NULLS NOT
DISTINCT` 네이티브 구문을 쓴다.

Model 타깃도 같은 선언을 EF Core fluent API로 반영한다(SQL Server `.HasFilter(null)`, PostgreSQL
`.AreNullsDistinct(false)`) — 그 김에, Model 타깃이 `### Indexes` 섹션의 **복합** `@unique`/
`@index` 선언 자체를 지금까지 전혀 배선하지 않고 있던 것(필드 단위 단일 컬럼만 처리)도 함께
바로잡았다.

### `@rollup`의 `where:` 절이 상관 서브쿼리 자신의 행 별칭을 더 이상 오염시키지 않는다

`where:` 절이 서브쿼리 자신의 상관 별칭을 직접 참조하면(예: `b.field = ...`) 그 별칭 토큰이
식별자 정규화 과정에서 다른 컬럼명과 똑같이 대소문자가 바뀌어 실제 별칭과 어긋났다 — 대소문자
구분 없는 콜레이션에서만 우연히 동작하던 상태. 별칭처럼 `.`으로 바로 이어지는 소문자 토큰은
더 이상 다시 쓰지 않는 일반 규칙으로 고쳤다.

같은 김에 부모 행(집계 대상 테이블 자신)의 컬럼을 참조하는 문서화된 방법
`$parent.<field>`(`field`는 m3l 필드명, snake_case)를 추가했다 — 렌더러가 실제로 쓰는 SQL
별칭에 의존하지 않는다.

## 0.26.0

### `M3L.Native` 0.8.0 — 관계 표기가 더 이상 타입 없는 필드로 새지 않는다

`- >author` 같은 필드 축 관계 표기(명세 §3.2.2·§3.2.4)를 파서가 이제 구조화해 낸다. 그
줄은 더 이상 `model.Fields`에 들어오지 않으므로, 이 생성기가 그 줄을 직접 판별하던 문자열
휴리스틱은 발화할 자리가 사라졌다 — 걷어내고 파서가 구조화한 필드(`raw`·`declaredIn`)를
그대로 읽는 것으로 대체했다. 로더 회계가 내는 문구(무엇을 썼는지, 관계는
`@reference(Target)`로만 읽는다는 안내)는 그대로다 — 달라진 것은 그 문구를 만드는 방법뿐,
동작도 여전히 `@reference(Target)`만 관계로 읽는다.

### 생성 C# 이 소비자의 `ImplicitUsings` 설정에 기대지 않는다

생성물이 컴파일되는지가 **소비 프로젝트의 설정에 달려** 있었고, 두 결함이 서로 **반대 방향의**
설정을 요구해 통과하는 구성이 없었다.

- **모델 타깃이 `using System;` 를 방출한다.** 생성 엔티티·인터페이스는 `Guid`·`DateTime` 을
  단순 이름으로 쓰면서 그 네임스페이스를 선언하지 않아, `ImplicitUsings` 를 끈 프로젝트에서
  `CS0246` 으로 깨졌다. 이제 파일이 자기가 쓰는 이름을 스스로 선언한다.
- **Api 타깃이 엔티티 타입을 소비자 네임스페이스로 완전 한정해 방출한다.** 종전에는 엔티티
  네임스페이스를 `using` 으로 들여왔고, `ImplicitUsings` 를 **켠** 프로젝트에서는 그것이 BCL
  이름과 부딪혀 `CS0104`(모호한 참조)가 났다 — 앞 항목과 정반대 설정을 요구한 것이 이 조합이다.
  이제 `using` 을 내지 않고 참조마다 한정하므로 어느 설정에서도 컴파일된다.
  ⚠ **생성 산출물의 텍스트가 바뀐다**(`Controllers_gen.cs`·`ApiRegistration_gen.cs` 등).
  **동작은 무변경**이고, 생성물을 리포에 커밋해 두는 프로젝트에서는 diff 가 발생한다.
- 별칭(`global using X = …`)이 아니라 완전 한정을 택한 이유: 별칭은 **어느 이름이 BCL 과
  부딪히는지의 목록**을 생성기가 이고 있어야 성립한다. 한정은 그 목록을 필요로 하지 않는다.

### 필드 목록에 쓴 관계 표기가 빌드를 죽이지 않는다

`>author` 같은 관계 표기가 `### Relations` 가 아니라 모델의 필드 사이에 쓰이면, 그 줄이
**타입 없는 필드**로 읽혀 생성이 실패했다. 이제 위치와 무관하게 관계로 읽고, 제자리가 아니라는
사실은 경고로 알린다 — 뜻이 모호하지 않은데 문서 전체를 거절할 이유가 없다.

### 문서 — 어떤 타깃도 읽지 않는 attribute

`docs/M3L.md` 에 「Attributes no target reads」 절이 생겼다. 언어가 정의하지만 이 생성기의 어느
타깃도 소비하지 않는 attribute 를 이름으로 나열한다 — 선언해도 산출물에 아무 흔적이 남지
않는다는 뜻이며, 그 사실은 **모델을 빌드해 산출물을 검사하는 테스트**가 지킨다. (패키지에
포함되지 않는 리포 문서다.)

## 0.25.0

### 폼 필드 하나만 다르게 그리는 `fieldOverrides`

생성 폼은 필드 렌더를 자기 안에서 끝내므로, *「이 필드 하나만 우리 컴포넌트로」*라는 요구의
유일한 탈출구가 `@slot` 이었다. 그 대가가 셋이다 — 슬롯 필드는 전폭 단독 행이 되어 레이아웃이
강제로 바뀌고, `@slot` 은 **모델 선언**이라 그 모델을 쓰는 소비자 전원이 슬롯을 공급해야 하며,
그 필드의 라벨·`required`·`error` 배선을 영구히 인수한다. 필드 하나를 다르게 그리는 대가로는
과하다.

```tsx
<OrderForm form={form} onChange={onChange} fieldOverrides={{
  vatType: ({ value, onChange, label, required, error }) => <VatTypePicker … />,
}} />
```

`fieldOverrides` 는 셋 다 없다. **컨트롤만 치환하고** 행 배치·라벨·`required`·`error` 는 생성기가
계속 소유해 렌더 함수의 `ctx` 로 넘긴다. 소비자가 정하는 것은 「이 `ctx` 로 무엇을 그릴지」뿐이다.

🔴 **키는 `keyof Entity` 가 아니라 `{Entity}FormField`** — *「이 폼이 실제로 컨트롤을 그리는
필드」*의 유니온이다. 슬롯 필드·파생 필드·오타가 **컴파일 오류**가 되어, 조용히 아무 일도 하지
않는 prop 이 생기지 않는다. 슬롯 필드를 뺀 이유는 그 필드가 **이미 소비자 것**이어서다 — 한
필드에 수단이 둘이면 어느 쪽이 이기는지가 새 계약이 된다. mapped type 이라 `value`/`onChange` 는
필드마다 타입이 다르다.

`@slot` 은 **존치한다.** 둘은 다른 것을 말한다 — `@slot` 은 「이 필드는 모델 차원에서 특별하다」
이고 그 선언은 폼 밖에도 도달하지만(`@immutable` 이 폼과 엔티티 양쪽에 도달하듯),
`fieldOverrides` 는 「이 앱에서만 다르게 그린다」이다. 표현력이 다른 둘을 어휘 수렴을 위해
합치지 않는다.

### 권한 키에 `delete` 동사 — 그리고 `0.24.0` 이 적은 근거의 정정

`0.24.0` 은 동사를 read·write 둘로 한정하며 *「런타임의 짝이
`RestrictPolicy(setName, readPolicy, writePolicy)` 라 `delete` 를 내면 소비하는 쪽이 하지 않는
구분을 발명하게 된다」*고 적었다. **그 서명은 심사된 적 없이 상속된 것이었고, 이제 셋을
받는다** — 런타임이 「고칠 수 있는 사람」과 「지울 수 있는 사람」을 가를 수 있게 됐다.
`Permissions_gen.cs` 는 이제 (엔티티셋 × 세 동사)마다 상수를 낸다.

🔴 **`Default` 매핑에는 delete 를 싣지 «않는다».** 런타임의 기본값이 「delete 는 write 정책이
governs」이고, 어느 셋이 그 둘을 가를지는 **앱의 결정**이다. 매핑에 delete 를 넣으면 그 결정을
전 엔티티셋에 대해 생성기가 대신 내리는 것이 되고, 그것은 키 **형식**을 입력으로 두는 이유와
같은 도메인 경계 침범이다. 매핑은 기계적 baseline 으로 남고, 상수는 앱이 실제로 가르는 셋에서
쓰인다 — **소비자의 구조분해 `(Read, Write)` 도 그대로다.**

### 읽기 필드에 쓰기축 제약을 싣지 않는다

`field_schema_gen` 은 `required` 를 파생 필드(`@lookup`/`@rollup`/computed)에서 제외하면서 그
이유를 주석으로 적어 두었는데, **같은 근거가 그대로 적용되는 `maxLength`·`min`·`max` 에는
게이트가 없었다.** 설계가 아니라 낙하다 — 넷 다 「사용자가 넣는 값」에 대한 진술이고 파생
필드는 사용자가 넣지 않는다.

증상은 조용하다: 이 맵을 순회하며 제약을 검사하는 소비자가 **고칠 수 없는 칸에** *「50자 이하여야
합니다」*를 띄우고, 그 사이에 서 있는 것은 소비자가 `readOnly` 로 먼저 거르느냐뿐이었다. 파생
필드는 이제 `derived`·`readOnly`·`label` 만 싣는다.

### 폼의 textarea 비우기 값이 컬럼의 nullability 를 따른다

NOT NULL `text` 컬럼의 `UTextarea` 가 비우기 값으로 **`null` 을 무조건** 방출했다. `Partial<T>` 의
해당 필드는 `string | undefined` 라, 생성물이 소비자의 `tsc --strict` 에서 거부되고 컬럼이 받지도
않을 `null` 을 보내려 했다. 날짜·숫자 분기가 이미 하던 대로 **nullability 를 따르게** 했다.

<sub>이 리포의 테스트는 생성물을 «문자열»로 본다 — 타입 검사를 통과하는지는 별개 축이라 657건이
전부 초록인 채로 남아 있었다.</sub>

### README — `field_schema_gen` 의 소비 계약

이 맵을 **무엇에 쓰는가**가 README 에 아예 없었다. 무엇이 실리는가, 읽기축과 쓰기축의 구분,
`readOnly` 로 거르는 순회, 그리고 **생성 폼은 이 맵을 읽지 않는다**는 사실(폼은 자기 안에서
같은 정보를 다시 만든다)을 절로 적었다.

## 0.24.0

### Api 타깃이 권한 키를 낸다 (`permissionKeyTemplate`)

생성기는 엔티티셋 목록을 이미 내고 있었다(`AddEntityPair`). 그 목록에 권한 차원이 없어서
소비앱이 같은 셋을 한 벌 더 손으로 유지했고, 거기서 한 줄이 빠지면 그 엔드포인트는 인증만
통과한 아무나 읽는다 — 빌드·타입·테스트·응답 어디에도 나타나지 않는다.

```json
{ "type": "Api", "…": "…", "permissionKeyTemplate": "{entitySetLower}.{verb}" }
```

`Api_gen/Permissions_gen.cs` — (엔티티셋 × 동사)마다 상수 하나 + 엔티티셋 → (read, write)
기본 매핑. 소비앱은 그 위에 예외만 얹으므로 **새 엔티티가 매핑에서 조용히 빠지는 일이
구조적으로 없어진다.**

🔴 **형식은 입력이다.** 키 모양은 소비앱의 인가 규약이라, 생성기가 정하면 규약이 다른
소비자에게 «틀린» 상수가 간다. 자리표시자는 `{entitySet}`·`{entitySetLower}`·`{verb}`·`{Verb}`
이고, **알 수 없는 자리표시자는 빌드 오류**다(통과시키면 어떤 정책과도 매칭 안 되는 리터럴 키가
조용히 나간다). 생략하면 아무것도 방출하지 않는다.

`@internal` 엔티티는 등록 파일과 같은 이유로 제외된다. 동사는 **read·write 둘뿐** — 런타임의
짝이 `RestrictPolicy(setName, readPolicy, writePolicy)` 라 `delete` 를 내면 소비하는 쪽이 하지
않는 구분을 발명하게 된다.

### 폼만 좁히는 `formsInclude` / `formsExclude`

`includeEntities`/`excludeEntities` 는 타깃의 **엔티티 파생 산출물 전체**에 걸린다. 그래서
*「타입은 전량 쓰는데 폼은 일부만 필요하다」*는 구성 — 접합 테이블·감사 로그·인라인 편집되는
자식처럼 단독 폼을 가질 수 없는 모델이 있는 앱 — 은 표현할 수단이 없었다. 좁히는 순간
`entities_gen.ts` 의 타입까지 사라졌기 때문이다.

```json
{ "type": "TypeScript", "outputPath": "../ui/src/types",
  "formsOutputPath": "../ui/src/forms",
  "formsInclude": ["Order", "Payment", "Consumer"] }
```

기존 필터와 **같은 술어·같은 검증**을 쓰고 적용 범위만 `{Entity}Form_gen.tsx` 로 좁다.

🔴 **폼 집합은 언제나 타깃 집합의 부분집합이다.** 생성 폼이 자기 타입을 `entities_gen.ts` 에서
임포트하므로, `includeEntities` 가 이미 제외한 엔티티를 `formsInclude` 에 적으면 임포트가 깨진
파일이 나온다 — 조용히 드롭하지 않고 **빌드 오류**로 세운다. `formsOutputPath` 없이 필터만
지정하는 것, `TypeScript` 아닌 타깃에 지정하는 것도 마찬가지로 오류다.

### `field_schema_gen.ts` 가 읽기 필드(lookup·rollup·computed)도 싣는다

지금까지 이 파일은 **쓰기 필드만** 실었다. 그런데 `entities_gen.ts` 는 읽기 필드를 타입에 싣고,
C# 읽기 클래스도 그 필드에 `[Display(Name=…)]` 를 낸다 — 즉 **모델에 있는 라벨이 한 타깃에는
도달하고 다른 타깃에는 도달하지 않았다.** 목록 표가 그리는 것이 대개 읽기 필드라, 그 열의
라벨은 소비앱이 손으로 적는 수밖에 없었고 모델 라벨을 고쳐도 표는 따라오지 않았다.

각 항목에 두 필드가 붙는다:

```ts
export type FieldConstraints = {
  // …
  derived?: 'lookup' | 'rollup' | 'computed'
  readOnly?: true
}

Order: {
  OrderNumber:  { required: true, maxLength: 30 },      // 그대로
  CustomerName: { label: '고객명', derived: 'lookup', readOnly: true },
  ItemCount:    { derived: 'rollup', readOnly: true },
}
```

⚠ **`Object.keys(FieldSchema.X)` 를 순회해 「입력 필드 목록」을 도출하고 있다면 그 결과가
넓어진다.** 읽기 필드는 편집 대상이 아니므로 `readOnly !== true` 로 걸러야 한다. 생성 폼
(`*Form_gen.tsx`)은 이 맵을 읽지 않으므로 영향이 없고, 저장 필드의 방출은 **한 글자도 바뀌지
않는다** — 순증이다.

읽기 필드에는 `required` 를 **싣지 않는다.** `@lookup(customer_id.name)` 이 NOT NULL 컬럼을
가리켜 non-nullable 로 해석되더라도, 그것이 폼더러 그 값을 받으라는 뜻은 아니다 — `required` 는
쓰기 축의 진술이라 파생 필드에 옮겨 적으면 거짓이 된다.

라벨이 없는 읽기 필드도 실린다. `derived`/`readOnly` 자체가 소비자가 알아야 할 정보이기
때문이다(저장 필드는 종전대로 제약이 하나도 없으면 생략된다).

### `@lookup` 이 대상 필드의 라벨을 상속한다

`@lookup(customer_id.name)` 은 **대상 필드가 이미 그 값을 이름 짓고 있다.** 같은 문자열을 두 번
적게 하는 것은 둘이 갈라지는 경로라, 라벨을 따로 안 붙인 lookup 은 이제 대상의 것을 물려받는다.

```markdown
## Customer
- name: string(50) @not_null "고객 상호"

## Order
- customer_name: string @lookup(customer_id.name)              # → label: '고객 상호'
- buyer:         string @lookup(customer_id.name) "매입처"      # → label: '매입처'  (직접 지정 우선)
```

- **직접 지정한 라벨이 언제나 이긴다.**
- **대상에도 라벨이 없으면 아무것도 지어내지 않는다** — 필드명을 PascalCase 로 바꿔 채우는
  폴백은 쓰지 않는다. `label` 의 존재는 「사람이 쓴 의미 있는 텍스트」라는 뜻을 유지한다.
- **`@rollup`·`@computed` 는 상속하지 않는다.** 집계(`count`·`sum(x)`)와 표현식에는 이름을 줄
  대상 «필드»가 없다.
- 상속은 **한 단계**다. 대상이 그 자신 라벨 없는 lookup 이면 거기서 멈춘다.

### 빌드가 「의도적으로 구현하지 않은 것」을 스스로 말한다

`time` 필드를 가진 모델이 TypeScript 폼 타깃에 닿으면 빌드가 안내 한 줄을 낸다 — 그 필드가
생성 폼에서 **자유 텍스트 입력**으로 나온다는 것, 그것이 결함이 아니라 **의도된 것**이라는 것,
그리고 근거를 어디서 읽는지(README 「타입 → 컨트롤 매핑」).

문서가 없어서 넣은 것이 아니다. 그 근거는 README 에 실측 표까지 갖춰 있었는데도, 생성물만 보고
「타입을 아는데 폼이 무시한다」로 읽고 직접 우회를 짜는 경로가 실재했다. 부족했던 것은 문서가
아니라 **그것을 만나는 시점**이라, 반드시 지나가는 자리인 빌드로 옮겼다.

**경고가 아니라 안내이고, stderr 이 아니라 stdout 이다.** 고칠 것이 없기 때문이다 — 경고로
내면 없애려 드는 것이 옳은 반응이 되어 버리고, 없앨 수 없으므로 결국 무시된다. 진단만 stderr
에 남으므로 `2>` 로 경고·오류만 거르는 파이프라인은 영향을 받지 않는다.

---

## 0.23.0

### enum 값에 `@deprecated` — 「이제는 고르지 않는 값」

값 레벨 `@deprecated` 가 `@system` 과 **같은 규칙**을 받는다: 생성 폼의 선택지에서 빠지고,
표시 라벨·C# enum·SQL CHECK 제약에는 **그대로 남는다.**

```markdown
## PaymentMethod ::enum
- card: "카드"
- voucher: "상품권" @deprecated("card 로 대체")
```

두 attribute를 하나로 합치지 않은 이유는 **이유가 곧 정보**이기 때문이다 — `@system`은
「시스템이 쓴다」, `@deprecated`는 「예전엔 골랐다」다. 폐지된 선택지를 `@system`으로 적으면
모델이 「누가 그 값을 쓰는가」에 대해 사실이 아닌 말을 하게 된다. 산출물은 동일하다.

인자(`@deprecated("card 로 대체")`)는 자유롭게 쓸 수 있고 생성기는 읽지 않는다.

> M3L은 enum 값 attribute를 기록만 하고 의미를 정하지 않는다(명세 §3.1.8) — 위 의미는 이
> 생성기가 정한 것이다. 파서 요구 버전은 `@system`과 같다(`M3L.Native` 0.6.0 이상).

⚠ **이 버전 전까지 `@deprecated`는 어떤 타깃도 읽지 않는 이름이었다** — 모델에 적혀 있어도
산출물에 아무 영향이 없었다(미등재 attribute는 경고 없이 통과한다). 다른 의도로(예: 린터·
사람이 읽는 메모) **이미 enum 값에 `@deprecated`를 붙여 두었다면, 이 버전부터 그 값이 생성 폼의
선택지에서 빠진다.** 재생성 후 해당 필드의 선택지를 확인할 것 — 저장·표시 라벨·C# enum·SQL
CHECK 제약은 그대로이므로 기존 행과 서버 경로는 영향받지 않는다.

판정은 **대소문자를 무시한 이름 일치**다(`@system`이 처음부터 그랬고 같은 술어를 쓴다) —
`@Deprecated`·`@DEPRECATED`도 같게 취급된다. 인자 유무는 보지 않는다.

### 🟠 `@system` 값을 가진 행을 편집하면 그 값이 선택지에서 사라졌다 (버그)

enum 값에 `@system`을 붙이면 생성 폼이 좁혀진 선택지 맵(`{Enum}SelectableLabels`)을 읽는다 —
그 값을 **새로 고를 수 없게** 하는 것이 목적이고, 그 목적은 지금까지 정확히 달성돼 왔다.

빠져 있던 것은 반대 방향이다. 같은 생성 폼이 기존 행도 바인딩하는데
(`{entity}FromEntity(row)`를 생성기가 함께 방출한다), **그 행이 이미 그 값을 들고 있으면**
컨트롤은 자기 선택지에 없는 값을 받는다. 즉 편집 화면에서 현재 값이 표시되지 않고, 그대로
저장하면 다른 값으로 바뀐다 — 저장은 제한하지 않는다던 `@system`의 계약과 어긋난다.

이제 좁힘을 **함수**로 방출한다. 현재 값이 제외된 값일 때만 그 하나를 되돌려 놓는다:

```ts
/** Input choices, plus the current value when the model excludes it (so editing an existing row keeps it). */
export function paymentMethodChoices(current?: string | null): Record<string, string>
```

```tsx
// 이전
options={enumToOptions(PaymentMethodSelectableLabels)}
// 이후
options={enumToOptions(paymentMethodChoices(form.Method))}
```

**생성 폼에 대해서는 소비앱이 할 일이 없다.** `enumToOptions` 헬퍼 시그니처는 인자 하나
그대로이고 (이 함수의 반환값이 그 인자에 그대로 들어간다), `{Enum}SelectableLabels`도 계속
방출된다. 제외된 값이 없는 enum의 산출물은 바이트 단위로 이전과 같다.

⚠ **생성 폼 밖에 손으로 만든 select 가 있다면 그쪽도 함께 옮겨야 한다.** 생성기가 닿는 범위는
생성물까지다 — 손으로 쓴 입력 자리가 전체 라벨맵(`{Enum}Labels`)을 펼치고 있으면, 모델에서
제외한 값이 **그 자리에서는 계속 선택지에 남는다.** `@system`·`@deprecated` 로 좁힌 enum을
입력에 쓰는 자리는 `{enum}Choices(current)`(편집 경로) 또는 `{Enum}SelectableLabels`(신규 전용)를
읽도록 옮길 것. 표시·필터 자리는 전체 라벨맵이 맞으므로 그대로 둔다.

⚠ **관측되는 변화**: 제외된 값을 가진 enum 필드의 폼 import가 `{Enum}SelectableLabels`에서
`{enum}Choices`로 바뀐다. 생성물끼리의 import라 소비앱 계약과는 무관하다.

---

## 0.22.0

### 생성 폼이 `timestamp`·`datetime` 필드에 `type="datetime"` 을 방출한다

이 두 타입은 그동안 아무 힌트 없는 자유 텍스트로 나갔다. 사용자가 `2026-08-19 10:00` 처럼
치면 서버 모델바인딩이 파싱에 실패해 400 이 나는데, 화면에는 어떤 형식이 필요한지 알려 줄
것이 없었다.

```diff
- <UInput label="발송 일시" value={form.SentAt ?? ''} onChange={v => onChange({ SentAt: v })} />
+ <UInput label="발송 일시" type="datetime" value={form.SentAt ?? ''} onChange={v => onChange({ SentAt: v || null })} />
```

**`datetime` 은 HTML input 타입이 아니다** — 명세의 invalid-value default 가 Text 라, 이 값을
그대로 `<input>` 에 넘기는 래퍼는 지금까지와 똑같은 텍스트 입력을 얻는다(Chrome 152 실측:
`.type === "text"`, 값 보존). 값을 담을 수 있는 위젯이 있는 래퍼만 이 토큰에서 그쪽으로
라우팅하면 된다. **네이티브 `datetime-local` 을 쓰지 않은 이유가 이것이다** — 그 컨트롤은
`DATETIMEOFFSET` 의 오프셋을 담지 못해 값을 통째로 버린다(같은 실측: `.value === ""`),
그래서 모르는 래퍼에게 데이터 손실이 된다. 실패 방향이 반대다.

`time` 은 **의도적으로 그대로** 자유 텍스트다. 날짜 피커는 시각-only 값을 다루지 않고,
`TIME(7)` 의 소수 초를 담는 컨트롤도 없다.

⚠ **같은 변경에 「비우기」 페이로드 변화가 포함된다.** 두 타입이 이제 `date`·숫자 필드와 같은
규약을 따른다 — nullable 필드는 `null`, 필수 필드는 `undefined`. 이전에는 빈 문자열(`''`)이
그대로 나갔고, nullable `DateTimeOffset` 컬럼을 비우려던 요청은 그 경로에서 400 을 받고
있었다.

**소비자 래퍼 요구조건**: `UInput` 의 `type?` 가 받을 수 있는 값에 `"datetime"` 이 추가된다.
모르는 값이어도 깨지지 않지만(위), 위젯을 붙이려면 그 위젯의 `value` 가 완전한 ISO-8601
`DateTimeOffset` 문자열(`YYYY-MM-DDTHH:mm:ss±HH:mm`)을 주고받아야 한다. 상세는 README
「소비 프로젝트 계약」.


### 따옴표 없는 속성 인자의 값이 더 이상 변형되지 않는다 (파서 핀 상승)

`M3L.Native` 핀을 올렸다. 이 생성기가 읽는 값이 **원문 그대로** 오게 되어, 산출물에 실려
나가던 변형이 사라진다. 코드 변경 없이 핀만으로 관측되는 차이라 여기 적는다.

콜론이 든 따옴표 없는 인자가 `key: value` 쌍으로 재작성되면서 **콜론 뒤에 공백이 삽입**되고
있었다. 그 값이 그대로 DDL 에 박혔다:

```sql
-- 이전
[Cutoff] NVARCHAR(20) NOT NULL DEFAULT N'23: 59:59',
-- 지금
[Cutoff] NVARCHAR(20) NOT NULL DEFAULT N'23:59:59',
```

시각·URI 스킴처럼 콜론이 값의 일부인 리터럴이 전부 해당한다. **이 형태의 `@default` 를 쓰고
있었다면 생성된 DEFAULT 제약이 틀린 값이었으므로, 재생성 후 이미 배포된 제약을 확인할 것.**

이 핀에는 따옴표 트림 관련 수정도 들어 있으나, **이 생성기의 산출물에서는 차이가 관측되지
않았다** — 대조 대상으로 삼은 형태(`@computed_raw` 의 백틱·큰따옴표 인자)가 두 판본에서 같은
DDL 을 냈다. 리포에 체크인된 모델로 다섯 타깃 158개 파일을 양쪽 핀으로 생성해 대조한 결과도
바이트 동일이다. 파서 층위의 변경 내역은 파서 자신의 CHANGELOG 를 볼 것.

### 어떤 타깃도 읽지 않는 `### Relations`·`### Behaviors`·`### Metadata` 도 회계에 포함

바로 아래 항목이 「언어가 정의하지 않은 섹션」만 대상으로 했던 것을, **언어가 정의한 섹션 중
아무도 읽지 않는 셋**까지 넓혔다. 근거는 추정이 아니라 실측이다 — 같은 모델을 이 세 섹션을
넣은 것과 뺀 것으로 각각 생성해 **다섯 타깃(Sql tsql·Sql postgres·Model·Api·TypeScript)의
산출물 19개 파일이 전부 바이트 동일**함을 확인했다. 세 섹션은 산출물에 아무 흔적도 남기지
않는다.

**섹션 중 소비되는 것은 `### Indexes` 하나뿐이다.** `@reference` 는 계속 소비된다(외래 키·
내비게이션·lookup 리다이렉트가 전부 여기서 나온다) — 읽히지 않는 것은 `### Relations` 섹션과
`@relation(...)` 속성 쪽이다. 동봉 명세 사본 §3.2·§3.3·§3.5·§4.3 에 이 구분을 적었다.

README 에도 「소비되지 않는 요소 회계」 절을 신설해 이 경고의 대상과 한계(속성 축은 대상이
아니다)를 밝혔다 — 지금까지 이 동작은 어디에도 문서화돼 있지 않았다.

### 언어가 정의하지 않은 `### 섹션` 이 무경고로 탈락하지 않는다

`### PrimaryKey`·`### Version`·`### Migration` 처럼 **언어가 정의하지 않은 섹션 이름**은 모델에
비구조 항목으로 실려 오지만 어떤 타깃도 읽지 않는다. 그런데 로더 회계가 `::view`/`::flow`/
extension 만 세고 있어서 이 섹션들은 **오류도 경고도 없이** 사라졌다 — `0.6.0` 에서 standalone
`::view` 에 대해 세운 「파싱된 모든 요소는 소비되거나 경고된다」 원칙의 미적용 구간이었다.

이제 같은 경고에 함께 나온다: `[m3l] 경고: 소비되지 않는 요소 2개 — … OrderItem: ### Version,
OrderItem: ### PrimaryKey`. **언어가 정의한 섹션(`### Indexes` 등)은 대상이 아니다** — 그것을
타깃이 소비해야 하는가는 기능 문제이지 「아무도 읽을 수 없는 이름」이 아니고, 소비 중인 섹션까지
경고에 섞으면 이 경고 자체를 무시하게 만든다.

### 스킴이 붙은 `@reference` 대상을 「없는 엔티티」로 말하지 않는다 (`MDD012` 신설)

`@reference(external://taxonomy.Category)` 처럼 스킴이 붙은 대상은 오타가 아니라 **이 생성기가
읽지 않는 표기**(M3L §5.2)인데, 지금까지 `MDD002`(대상 엔티티 없음)로 보고돼 **없는 이름의
오타를 찾게 만들었다.** 모델 이름에는 콜론이 올 수 없으므로, 대상에 콜론이 있으면 그것은 이름이
아니라 스킴이다 — 이제 `MDD012` 로 스킴 이름을 되짚어 주고, 이 빌드가 소유하지 않은 테이블을
가리키려면 `@reference` 없이 컬럼만 선언하면 된다는 것까지 알린다. `MDD002` 는 진짜 오타에만
남는다.

### 동봉 명세 사본이 「생성기가 어디서 멈추는지」를 절마다 밝힌다

`docs/M3L.md` 는 언어 명세의 사본인데, **어느 절이 이 생성기에서 동작하지 않는지를 말하지
않았다.** 그래서 사본만 읽은 소비자가 §5.2 External Schema References(`@reference(external://…)`)
를 지원 기능으로 읽고, 직접 스파이크를 짜서야 미지원임을 알게 되는 일이 실제로 일어났다.

`95961df` 가 축약 타입 절 하나에 대해 세운 원칙 — *번들 사본이 읽히는 단 하나의 이유는
「구현이 어디서 멈추는가」다* — 을 문서 전체에 적용했다. 문서 머리에 이 사본의 성격과 정본
위치를 밝히는 안내를 두고, 생성기가 동작시키지 않는 **8개 절** 각각에
**「What this generator implements」** 노트를 달았다: §4.1 의 다중행 `### PrimaryKey` 형식 ·
§4.5 조건부 필드 · §4.6 중첩 구조의 한계 · §4.7 검증 규칙 · §4.8 템플릿/제네릭 ·
§5.2 외부 스키마 참조 · §6.1 스키마 버저닝 · §6.2 마이그레이션 표기.

각 노트의 서술은 실제 파서 출력으로 확인했다 — 이들 대부분은 **오류도 경고도 없이** 통과한다.
`### PrimaryKey`·`### Version`·`### Migration` 은 모델에 비구조 항목으로 실려 오지만 아무 타깃도
읽지 않고, `@if`·`required`/`visible`·`@validate` 는 평범한 속성으로 실려 와 역시 읽히지 않는다.
`## List<T>` 는 이름이 문자 그대로 `List<T>` 인 모델이 된다. §5.2 만 예외로 빌드를 세운다 —
`external://…` 를 없는 모델 이름으로 보고 미해결 참조 오류를 낸다. §5.2 노트는 그 대신 지금
쓸 수 있는 방법(그 컬럼을 `@reference` 없이 선언 — FK·내비게이션·lookup 리다이렉트 없는 평범한
컬럼이 나온다)과 그때 무엇을 잃는지도 함께 적는다.

### 동봉 명세 사본의 닫히지 않은 코드펜스 수정

같은 문서 §3.2.1 의 코드블록 하나가 닫히지 않아, **그 지점부터 문서 나머지가 뒤집혀 렌더링됐다**
— 설명 문단이 코드로, 예제가 문단으로. 이 절 이후를 GitHub 에서 읽은 사람은 계속 그 상태를 봤다.
위 노트 8개가 전부 그 뒤에 있어 이 수정 없이는 노트도 코드블록 안에 묻혔을 것이다.

## 0.21.0

### Added

- **The PostgreSQL Sql target now emits views for Lookup/Rollup derived fields and
  soft-delete**, symmetric with the T-SQL target. A model with a `@lookup`/`@rollup` field now
  gets a `{table}_full_view.sql` in a new `views_gen/` directory; a model with `deleted_at` also
  gets `{table}_ud_view.sql`. The Model target's generated `ToView(...)` mapping and derived
  column mapping now point at these actual view names instead of a name nothing created — a
  model using either now works end to end under `dialect: postgres`, including through OData.
- Computed derived fields and an `@indexed` Rollup remain unsupported under the PostgreSQL
  dialect (expression syntax differs by dialect; a materialized view needs its own refresh
  strategy) — a model that needs either, or that chains a Lookup/Rollup through one, still gets
  a build-time `stderr` warning instead of a silently broken view.

### Fixed

- The PostgreSQL Model target's `ToView(...)` mapping for a FullView/UdView-backed Ext entity
  used the T-SQL target's PascalCase name (`{Model}FullView`) even though this dialect commits to
  unquoted snake_case identifiers everywhere else, and never mapped a Lookup/Rollup property's
  physical column name at all — both silently wrong since no such view had ever existed to expose
  the mismatch.

## 0.20.0

### Added

- **Generated forms can now surface a per-field validation message.** `{Entity}FormBase` accepts
  a new `errors?: Partial<Record<keyof {Entity}, string>>` prop, and every rendered control
  (`UInput`/`UTextarea`/`USelect`/`UCheckbox`) now also receives its own `error?: string` — a new
  requirement on the consumer's component library, documented in the README consumer contract
  alongside `disabled`. How a component turns that string into a visible validation state is left
  entirely to it; the generator makes no assumption about the underlying mechanism.

## 0.19.1

### Fixed

- **A conditional `@rollup`'s `where:` clause is now honored when rendering the generated view's
  correlated subquery.** The filter was silently dropped, so a rollup like an "active order count"
  that should exclude cancelled rows was rendered unfiltered instead — counting or summing over
  every row regardless of the documented condition.

## 0.19.0

### Added

- **`mdd build`가 실행할 때마다 더 최신 버전이 nuget.org에 있는지 알려준다.** 조회는 최대
  24시간에 한 번만 하도록 로컬에 캐시하고(연속 실행이 매번 네트워크를 타지 않는다), 2초
  타임아웃을 넘기거나 실패하면 조용히 넘어간다 — 오프라인이거나 응답이 느려도 빌드 자체는
  전혀 영향받지 않는다. 실제 업데이트는 여전히 표준 경로(`dotnet tool update -g mdd`)가
  하며, 이 기능은 그 사실을 안내만 한다. `MDD_NO_UPDATE_CHECK` 환경변수로 완전히 끌 수
  있다(기존 `MDD_DEBUG`와 같은 관례).

### Fixed

- **nullable number 필드의 폼 클리어가 OData PATCH에 반영되지 않던 결함 수정** — 생성 폼
  (`*_gen.tsx`)의 number 컨트롤이 값을 지울 때 필드의 nullable 여부와 무관하게 항상
  `undefined`를 보냈다. `JSON.stringify`가 `undefined` 프로퍼티를 직렬화에서 통째로 생략해,
  PATCH 바디에 그 필드가 아예 빠지고 OData delta 시맨틱상 "손대지 않음"으로 해석돼 클리어가
  조용히 무시됐다. `date`/`select`/`textarea` 컨트롤이 이미 쓰던 nullable/required 구분
  패턴(nullable → `null`, required만 `undefined`)을 number 컨트롤에도 적용해 해소.

## 0.18.1

### README — Model/Sql 타깃 소비 시 주의사항 안내

README의 "빌드 실행" 절(Model 타깃 퀵스타트 직후)에 두 가지를 추가했다: ⑴ Model 타깃 생성
엔티티는 런타임 타입을 단순 이름으로만 참조하므로 소비 프로젝트가 직접 `global using`을
선언해야 한다는 것 — 빠뜨리면 `CS0246`으로 드러난다. ⑵ Sql 타깃의 `Scripts_gen/`은 SSDT
post-deployment 관례라, desired-state 스타일 스키마 관리 도구는 이를 스킵하므로 별도 배포
단계로 분리해야 한다는 것. 둘 다 문서만 추가, 생성기 동작 변경 없음.

### README의 특정 제품 지목 문구 일반화

README 네 곳이 "선언형 도구"의 예로 특정 도구 이름을 직접 지목하고 있었다 — 이 생성기는
그 도구의 존재를 알아서는 안 되므로("생성 타깃" 표의 소비자 열, `emitEnumCheckConstraints`
설명, postgres Sql 타깃 관례, `samples/` 설명), 전부 "desired-state 스타일 스키마 관리
도구"로 통일했다. 문서만 변경, 생성기 동작 변경 없음.

### CLI 최상위 예외 출력 정리

`mdd build`가 실패할 때 원시 .NET 스택트레이스(~10줄)가 그대로 출력되던 문제를 고쳤다 —
이제 예외 메시지 한 줄만 출력한다. JSON 스키마 오류는 이미 메시지 자체에 `Path`/`LineNumber`/
`BytePositionInLine`이 포함돼 있어 정보 손실이 없다. 스택트레이스가 필요하면 `MDD_DEBUG`
환경변수를 설정한다(`MDD_DEBUG=1 mdd build`).

## 0.18.0

### `mdd.json` JSON Schema

에디터가 JSON Schema를 이해하면(예: VS Code) `mdd.json`을 저장 즉시 검증하고 타깃별 옵션을
자동완성할 수 있다. 모든 계층에서 `additionalProperties: false`를 강제해, 타깃 레벨 옵션을
설정 루트에 잘못 둔 경우(System.Text.Json이 알 수 없는 속성을 기본적으로 무시하므로 조용히
무시된다) 가장 흔한 실수 유형을 즉시 드러낸다. `$schema` 필드에
`https://raw.githubusercontent.com/iyulab/mdd-booster/main/schemas/mdd.schema.json`을 추가하면
바로 쓸 수 있다.

README의 `emitForeignKeyIndexes` 기본값 표기 오류도 함께 정정했다 — `0.13.0`에서
`false`→`true`로 바뀐 뒤 세 릴리스 동안 문서가 낡은 값을 그대로 두고 있었다.

### 실행 가능한 퀵스타트 샘플

README의 Model 타깃 퀵스타트가 실행 가능한 예제를 가리키지 않고 있었다. `samples/`에
`@reference`/`@lookup`/`@rollup`/`@computed`/enum을 쓰는 Project/Task 모델과 동작하는
`mdd.json`(Sql/Model/Api 타깃)을 추가했다 — `mdd build ./samples`로 바로 검증할 수 있다.
생성 결과물은 커밋 대상이 아니다(재생성되는 산출물).

### 파괴적 변경 없음

두 추가 모두 기존 모델·설정의 생성 결과에 영향을 주지 않는다.

## 0.17.1

### 순환 FullView 의존성 에러 메시지 — 필드 단위 귀속

`0.17.0`의 build-time 순환 감지는 순환 경로를 모델 이름만으로 나열했다(`Order -> OrderItem
-> Order`). Lookup이 많은 모델에서는 어느 필드가 각 홉을 만들었는지 역추적해야 했다. 이제
각 홉에 그 홉을 만든 필드와 종류가 붙는다: `Order -> OrderItem (via Order.SupplyTotal rollup)
-> Order (via OrderItem.CustomerName lookup)`. 순환 감지 로직 자체는 바뀌지 않았다 — 같은
그래프에서 귀속 정보만 함께 기록한다.

## 0.17.0

### 체이닝 Lookup + rollup 역방향 순환 뷰 의존성 — build-time 조기 실패

한 모델이 다른 모델의 파생(rollup) 필드를 lookup으로 체이닝하고, 그 다른 모델이 다시 앞
모델의 파생 필드를 되짚는 경우, `FullViewRenderer`가 모델 단위로는 각각 옳게 판단한 리다이렉트
두 개가 합쳐져 모델 간 순환을 만들 수 있었다. 지금까지는 `mdd build`가 이를 통과시켰고,
Schemorph 같은 선언형 스키마 도구가 실제로 뷰를 배포하려 할 때에야 SQL72009로 처음 드러났다.

이제 `mdd build`가 뷰를 쓰기 전에 순환을 그래프로 탐지해 순환 경로를 이름으로 나열하며
즉시 실패한다 — 배포 시점이 아니라 빌드 시점에 잡힌다. 자동 우회(순환 지점을 베이스
테이블+원 lookup 대상으로 재라우팅)는 이번 릴리스에 포함되지 않는다 — `IsDerivedColumn`
판정이 이미 한 번 회귀를 낸 민감한 지점이라 별도 트랙으로 신중히 다룬다.

### 생성 폼 `FormSection` — `sectionProps` 패스스루

소비 컴포넌트(`@iyulab/enterprise`의 `FormSection`)가 이미 받는 `className`/`style`을
생성기가 호출부에서 넘길 방법이 없었다. 이제 각 엔티티마다 `{Entity}FormSectionProps`
타입을 방출하고, 생성 폼(`{Entity}FormBase`)이 `sectionProps?: {Entity}FormSectionProps`를
받아 섹션 제목별로 `className`/`style`을 해당 `<FormSection>`에 그대로 전달한다. 소비앱은
이걸로 CSS 기반의 접기/숨김을 스스로 구현할 수 있다. 내장 접힘 시맨틱(`advancedSections`,
`@group`의 collapsible 옵션)은 이번 릴리스에 포함되지 않는다 — 실사용 사례가 하나뿐이라
UX(펼침 상태 기억 위치, 접힌 섹션의 검증 타이밍)를 지금 굳히기엔 이르다고 판단했다.

> **`className`/`style`은 새로 추가된 요구조건이다** — `sectionProps`를 넘기는지와 무관하게
> 모든 생성 폼의 `<FormSection>` 호출에 항상 방출되므로, 소비 중인 `FormSection` 컴포넌트가
> 이 두 프롭을 받지 못하면 업그레이드 즉시 컴파일이 깨진다. `disabled`(0.16.0)와 같은 종류의
> 요구조건 추가다.

---

## 0.16.0

### `@immutable` — 생성 폼에서 `disabled`, 생성 엔티티에서 `[Editable(false)]`

`@immutable`은 표준 필드 속성 카탈로그에 이미 등록돼 있었으나 지금까지 어느 타깃도 소비하지
않고 있었다.

**TypeScript 타깃**: 생성 폼의 네 컨트롤(`UInput`·`UTextarea`·`USelect`·`UCheckbox`) 전부에
`disabled` prop을 방출한다 — 필드는 계속 보이고 현재 값에 바인딩된 채로, 편집만 막힌다.
`readOnly`가 아니라 `disabled`를 선택한 이유: 체크박스·셀렉트는 네이티브 `readOnly` 의미가
컴포넌트마다 일관되지 않지만 `disabled`는 넷 다 동일하게 지원한다.

> **`disabled`는 새로 추가된 요구조건이다** — 소비 중인 컴포넌트 래퍼가 이 prop을 받지
> 못하면 갱신해야 한다.

**Model 타깃**: 생성 엔티티(쓰기·읽기 클래스 둘 다)에 `[Editable(false)]`를 방출한다 — TS
쪽과 같은 메타데이터일 뿐, 이 리포가 생성하는 어떤 API 계층도 이를 강제하지 않는다(두 타깃
모두 동일).

### `@unique`/`@index` → 생성 엔티티의 `HasIndex()`

SQL 타깃이 이미 UNIQUE 제약·인덱스로 방출하던 필드 단위 `@unique`/`@index`가 이제 Model
타깃에도 도달한다. `OnModelCreating`에 `modelBuilder.Entity<T>().HasIndex(...)`(`@index`) /
`.HasIndex(...).IsUnique()`(`@unique`)를 방출하며, 인덱스·제약 이름은 SQL Server(`UK_`/`IX_`)·
PostgreSQL(`uq_`/`ix_`) 방언이 실제로 쓰는 이름과 정확히 일치한다. 한 필드가 둘 다 선언하면
`unique`만 방출된다(제약이 이미 인덱스를 소유하므로 SQL 타깃과 동일한 배제). PK 필드는
제외된다(이미 유일 인덱스를 갖는다).

**섹션 레벨 복합 선언**(`@unique(c1, c2)`, `@index(c1)`)은 이번 릴리스에 포함되지 않는다 —
필드 단위 선언만 방출한다.

---

## 0.15.0

### 필드 단위 `@internal` — 저장하되 노출하지 않는다

`@internal`은 지금까지 모델(엔티티) 단위로만 쓸 수 있었다. 이제 필드에도 붙는다: 표시된
필드는 기반 테이블의 실컬럼, 쓰기 엔티티의 실프로퍼티로는 그대로 남지만, 읽기 표면에서는
빠진다 — `Sql` 타깃은 `FullView`/`UdView`의 `SELECT` 목록에서, `Model` 타깃은 인터페이스와
`*Ext`(읽기) 클래스에서 제외된다(`*Ext` 는 그 인터페이스를 구현하므로 인터페이스에서도 함께
빼야 컴파일된다). 파생 필드(Lookup/Rollup/Computed)에도 같은 술어가 적용되며, 그 결과 참조하는
FK가 오직 내부 전용 lookup에만 쓰였다면 해당 JOIN 자체가 뷰에서 사라진다.

두 타깃 모두 한 번의 빌드 안에서 같은 `mdd.json`이 만든 같은 `ResolvedModel`의 같은 필드
데이터에 같은 술어를 적용하므로, `Sql`과 `Model`의 짝이 서로 다른 시점에 어긋나는 경우는
생기지 않는다.

**`TypeScript` 타깃(생성 폼)은 이번 릴리스에 포함되지 않는다.** 표시된 필드는 계속 생성 폼에
나타난다 — 그 타깃까지 확장할지는 아직 미결이다.

`@internal`이 붙지 않은 필드의 생성 결과는 바이트 단위로 동일하다.

> **이미 배포된 `FullView`/`UdView`를 재생성하면 뷰 정의 자체가 바뀐다** — 필드를 새로
> `@internal`로 표시하면 그 컬럼이 `SELECT` 목록에서 빠지므로, 선언형 스키마 도구는 이를
> 뷰 재정의로 감지한다. 이는 이 기능이 만드는 새로운 위험이 아니라 뷰 `SELECT` 목록을 바꾸는
> 모든 모델 변경이 이미 갖는 배포 특성이다 — 재생성 전에 `diff`로 실제 변경 범위를 먼저 볼 것.

### `@label`이 필드 라벨을 명시적으로 오버라이드한다

`@label`은 파싱만 되고 어느 타깃도 읽지 않았다 — 라벨로 쓰이는 텍스트는 필드 설명(description)
뿐이었고, 설명이 없을 때 세 타깃이 서로 다르게 행동했다(`Model`은 `[Display]` 자체를 생략,
생성 TS 필드 스키마는 `label: null`, 생성 폼은 PascalCase 프로퍼티 이름으로 대체). 이제
`@label(text) → description → PascalCase 필드 이름` 우선순위가 공유 정본이고, 생성 폼은 이
정본을 직접 읽는다(입력 필드는 항상 어떤 라벨이든 표시해야 하므로).

`Model`의 `[Display(Name)]`과 TS 필드 스키마의 `label`은 계속 **명시된 것이 있을 때만**
채워진다(`@label(text) ?? description`, PascalCase 대체 없음) — 둘 다 원래 "적을 게 없으면
속성/필드 자체를 생략한다"는 기존 계약을 그대로 유지한다. 기계적으로 되풀이한 프로퍼티
이름은 그 두 자리에서는 정보가 아니라 잡음이기 때문이다.

`@label`을 쓰지 않는 필드의 생성 결과는 바이트 단위로 동일하다.

### `@help`가 `[Display(Description)]`로 방출된다

필드의 `@help` 텍스트가 이제 생성 엔티티의 `[Display(Description = "...")]`로 나온다 — 기존
`@label → Name`, `@group → GroupName` 매핑과 같은 자리, 같은 규칙이다: `[Display]`는
Name/GroupName/Description 중 하나라도 있을 때만 방출되고, 셋 다 없으면 생략된다.

### 파괴적 변경 없음

세 기능 모두 새 M3L 속성을 쓸 때만 생성 결과가 바뀐다. 기존 모델을 재생성해도 산출물은
동일하다.

---

## 0.14.0

### 🟡 절대경로 `projectPath`/`outputPath`가 다른 git 작업트리를 가리킬 때 경고

`mdd.json` 타깃이 절대경로(또는 `..`를 많이 쓴 상대경로)로 프로젝트 루트를 지정하면, 그 값은
`mdd build`를 어느 디렉터리에서 실행하든 항상 같은 곳을 가리킨다. 이 설정을 여러 물리적
체크아웃(예: linked git worktree와 그 메인 체크아웃, 또는 별도 클론 두 개)에서 그대로
재사용하면, 빌드는 지금 실행 중인 체크아웃이 아니라 그 경로가 실제로 위치한 체크아웃에
조용히 쓴다.

이제 `mdd build`가 호출된 디렉터리와 해석된 경로가 서로 다른 git 작업트리에 속하면 `stderr`로
경고한다. `Sql`·`Model`·`Api`·`TypeScript` 타깃 전부 적용된다(`projectPath`/`outputPath`/
`formsOutputPath`/`sqlProjectPath`). 경고이며 빌드를 막지 않는다 — 여러 체크아웃이 의도적으로
한 위치를 공유하는 구성일 수도 있다.

---

## 0.13.2

### 🔴 TS enum 생성기가 wire 이름이 아니라 CLR 스타일 이름을 방출했다 (버그)

`types/enums_gen.ts`의 union 리터럴과 `enum_labels_gen.ts`의 라벨/선택-맵 키가 M3L 멤버 이름을
PascalCase로 변환해 방출하고 있었다 — C# 생성기(`Enum_gen/*.cs`)가 이미
`[EnumMember(Value = "...")]`로 선언하는 wire 이름(M3L 원문 그대로, 예: `in_production`)과 다른 값이었다. 서버 쪽 EDM/JSON 계층이 `[EnumMember]`를 무시하는 동안에는
두 산출물이 우연히 같은 모양(CLR 이름)으로 맞아떨어지지만, 그 계층이 `[EnumMember]`를 따르는
순간 TS 쪽만 어긋난 값을 계속 주장하게 된다 — 같은 enum 선언의 두 생성물이 서로 다른 wire
계약을 주장해서는 안 된다는 것이 이 수정의 근거다.

TS 생성기가 C#과 같은 wire-이름 해석(멤버 선언 그대로)을 쓰도록 고쳤다 — 두 산출물이 같은
enum 선언에서 항상 같은 wire 계약을 가리킨다.

같은 원인이 폼 생성기에도 있었다: `@default(...)`가 붙은 enum 필드의 빈 상태 초기값
(`empty{Entity}` 상수)이 default 값을 PascalCase로 바꿔 넣고 있었다 — union 타입 자체가
wire 이름으로 바뀌면 그 리터럴은 더 이상 타입의 멤버가 아니게 되어 `tsc` 오류가 난다. 같은
default가 C# 쪽에서 PascalCase 멤버 표현식(`Status.Draft`)으로 옳게 렌더링되는 것과는 다른
자리 — TS 쪽은 멤버 표현식이 아니라 리터럴이므로 wire 이름 그대로 써야 한다.

### 파괴적 변경 — 재생성 시 TS 산출물 값이 바뀐다

`enums_gen.ts`의 union 리터럴, `enum_labels_gen.ts`의 객체 키·`Exclude<>` 타입 인자, 그리고
`@default(...)`가 붙은 enum 필드의 폼 빈 상태 초기값이 CLR 스타일(`'InProduction'`)에서
M3L 원문(`'in_production'`)으로 바뀐다. 라벨 텍스트 자체(사람이 읽는 문자열)는 바뀌지 않는다
— description이 없는 값의 표시 폴백만 그대로 PascalCase를 쓴다. 이 값을 문자열 비교하는
소비앱 코드(예: `status === 'InProduction'`)는 재생성 후 갱신이 필요하다 — 서버가
`[EnumMember]`를 따르는 한, 새 값이 실제 wire 계약과 일치한다.

## 0.13.1

### 🔴 enum CHECK 제약(opt-in)이 apply→diff 를 영원히 수렴하지 않게 만들었다 (버그)

`emitEnumCheckConstraints` 는 처음 나온 순간부터 이 문제 때문에 기본 off 였다. 근본 원인을
라이브 SQL Server 로 실측했다: **SQL Server 는 `CHECK (col IN (v1, v2, …))` 를 문자 그대로
저장하지 않는다** — 제약이 만들어지는 순간 **역순 OR 체인**으로 다시 쓴다
(`sys.check_constraints.definition` 이 `CHECK (col IN (a, b))` 를
`([col]=b OR [col]=a)` 로 돌려준다. 값이 하나면 그냥 `=` 로 접힌다). 선언형 비교 도구가
그 라이브 정의를 우리 `IN (…)` 원본과 비교하면 **영원히 다른 모양으로 보여**, 몇 번을
apply 해도 재-diff 가 수렴하지 않았다.

생성기가 처음부터 SQL Server 가 실제로 저장하는 모양(역순 OR 체인)으로 방출하도록 고쳤다.
라이브 SQL Server 2022 컨테이너로 처음부터 끝까지 검증: 기존 테이블에 제약을 새로 추가 →
apply → 재-diff 가 **첫 시도에 변경 0건**(이전에는 같은 DROP+ADD 가 계속 반복됐다).

**Postgres 타깃은 이 수정의 영향을 받지 않는다** — 이미 자기만의 재작성 형태
(`= ANY(ARRAY[...])`)로 저장하며, 그쪽이 같은 문제를 겪는지는 별도 검증이 필요해 이번에는
다루지 않았다.

### 파괴적 변경 없음

`emitEnumCheckConstraints` 를 켠 소비자는 재생성 시 제약 정의 텍스트가 바뀐다(같은 의미의
다른 모양) — 다음 apply 한 번으로 반영되고, 그 apply 이후로는 재-diff 가 안정된다. 옵션
자체는 여전히 기본 off — 이 릴리스는 이유였던 결함만 고쳤을 뿐, 기본값 재검토는 별개다.

---

## 0.13.0

### ⚠️ `emitForeignKeyIndexes` 기본값이 `false` → `true`로 바뀐다 (Sql·Postgres 타깃, 동작 변화)

`@reference` FK마다 EF는 관례상(`ForeignKeyIndexConvention`) 인덱스를 만들지만, Sql/Postgres
타깃은 이 옵션이 꺼져 있는 한 만들지 않았다 — 같은 선언을 두 타깃이 서로 다른 스키마로
번역하고 있었다. 실측(소비자 1곳, 43모델·51`@reference` 기준): 실제로 방출되는 인덱스는
**11개(≈17%)** — 나머지는 PK/UNIQUE/복합 인덱스의 선행 컬럼으로 이미 덮여 있었다. 켜기·끄기
둘 다 `hasDestructiveChanges=false`(데이터 손실 없는 인덱스 생성/삭제)로 확인됐다.

**기본값을 켜서 두 타깃을 정렬한다.** 재생성하면 이전에 커버되지 않았던 FK 컬럼마다
인덱스가 새로 방출된다 — 정확히 몇 개인지는 모델마다 다르다(위 11개는 한 소비자의 실측값이지
일반 비율이 아니다).

**되돌리려면**: `mdd.json`의 해당 타깃에 `"emitForeignKeyIndexes": false`를 명시하면 0.12.x
동작(인덱스 미방출)으로 복귀한다.

**배포 전 확인할 것**: schemorph 같은 선언형 도구를 쓰는 소비자는 재생성 후 `diff`(또는
`diff --format sql`)로 실제로 몇 개가 어디에 생기는지 먼저 본다 — 인덱스 생성/삭제는
`--allow-destructive` 게이트에 걸리지 않으므로 예상 밖의 인덱스가 있어도 조용히 적용된다.
schemorph 최소 버전은 `>= 0.7.0`(그 미만의 postgres provider는 인덱스 차이를 `compare_failed`
로 거절해 테이블·컬럼 비교까지 함께 실패한다).

### 파괴적 변경 없음

기본값 전환 자체는 스키마에 인덱스를 추가할 뿐 데이터를 건드리지 않는다. 옵션을 껐다가
되돌리는 것도 비파괴적으로 확인됐다(위 참조).

---

## 0.12.4

### 🔴 0.12.3의 체이닝 lookup 수정이 자기참조·상호참조 모델에서 배포 불가능한 순환 뷰 의존성을 만들었다 (회귀)

0.12.3은 `@lookup(fk.field)`의 대상 컬럼이 파생(Lookup/Rollup/Computed) 필드일 때 JOIN을
대상의 `{Target}FullView`로 돌리도록 고쳤지만, 그 조건을 "요청한 컬럼이 실제로 파생 컬럼인가"
가 아니라 "대상 모델이 FullView를 하나라도 갖는가"로 넓게 적용했다. 그 결과:

- **자기참조 FK**(예: `parent_id: identifier? @reference(SameModel)?`)의 lookup이 **raw**
  베이스 컬럼(`name` 등)을 가리켜도 그 모델 자신의 FullView와 JOIN — 뷰가 자기 정의 안에서
  자기 자신을 참조해 배포 순서를 계산할 수 없었다.
- **Rollup과 역방향 Lookup이 같은 두 모델 사이에 있으면** 순환이 생겼다 — Rollup 서브쿼리는
  이미 대상에 파생 필드가 있으면 무조건 `{Target}FullView`를 소스로 삼고 있었는데(0.12.2
  이전부터), 여기에 반대 방향 Lookup까지 같은 넓은 조건으로 FullView를 향하면 A↔B 순환이
  완성됐다. `count` 집계처럼 대상 컬럼을 전혀 읽지 않는 Rollup도 마찬가지로 불필요하게
  FullView로 리다이렉트됐다.

`mdd build`는 두 경우 모두 에러 없이 성공하지만, 생성된 뷰 정의를 배포하는 선언형 스키마
도구는 순환 의존성을 거부한다.

고쳐서 조건을 정확하게 좁혔다 — Lookup JOIN과 Rollup 서브쿼리 둘 다, **요청한 그 컬럼이
대상 모델 자신의 Lookup/Rollup/Computed 필드일 때만** `{Target}FullView`를 향한다. raw
베이스 컬럼을 읽는 경우(자기참조 포함)와 `count` 집계는 항상 베이스 테이블을 그대로 쓴다 —
0.12.2 이전과 동일한 동작이다. 체이닝된 파생 컬럼 lookup(0.12.3이 원래 고치려던 경우)은
그대로 FullView를 향한다.

### 파괴적 변경 없음

수정은 순환을 만들던 자리에서만 생성 결과를 바꾼다(리다이렉트를 되돌려 베이스 테이블을
가리키게 함). 체이닝된 파생 컬럼 lookup의 생성 결과는 0.12.3과 동일하다. 재생성 시 자동으로
반영된다.

---

## 0.12.3

### 🔴 슬롯 플레이스홀더 라벨이 중괄호를 포함하면 생성 폼이 파싱 실패했다 (버그)

`@reference`/`@slot` 필드의 슬롯 플레이스홀더는 필드 라벨을 JSX 텍스트 위치에 그대로
삽입했다. 라벨에 `{`나 `}`가 있으면 그 문자가 JSX 표현식 컨테이너로 재해석되고, 그 안의
콤마는 JS 콤마 연산자로 파싱되어 빌드 도구가 명시적으로 거부했다 — `tsc --noEmit` 은 통과하고
번들러 트랜스폼 단계에서만 드러나는 종류의 실패였다.

같은 파일의 다른 모든 컨트롤은 라벨을 속성값(`label="..."`)으로 넣어 이미 안전했다 — 텍스트
노드로 나가는 슬롯 플레이스홀더 한 자리만 안전하지 않았다. 라벨을 JS 문자열 리터럴로 감싸는
것으로 고쳤다. 렌더링 결과는 이전과 동일한 텍스트다.

### 🔴 체이닝된 lookup 이 Lookup 필드를 다시 가리키면 생성된 뷰가 존재하지 않는 컬럼을 참조했다 (버그)

`@lookup(fk.field)` 에서 `field` 가 대상 모델 자신의 Lookup(또는 Rollup/Computed) 필드일 때,
생성된 `{Name}FullView` 의 JOIN 은 여전히 대상의 **베이스 테이블**을 향했다 — 그 컬럼은
베이스 테이블이 아니라 대상의 `{Target}FullView` 에만 존재한다. `mdd build` 는 에러 없이
성공했지만, 뷰 정의를 검증하는 선언형 스키마 도구는 존재하지 않는 컬럼 참조를 거부했다.

Rollup 서브쿼리가 이미 같은 상황(대상이 파생 필드를 가질 때 `{Target}FullView` 를 소스로
쓰는 것)을 처리하고 있었다 — Lookup JOIN 빌드만 그 처리를 따르지 않고 있었다. 같은 처리를
재사용해 고쳤다.

### 파괴적 변경 없음

두 수정 모두 재생성 시 자동으로 반영된다. 설정 변경도 소비자 조치도 없다.

---

## 0.12.2

**생성기 동작은 바뀌지 않는다** — 문서와 테스트뿐이다. 재생성한 산출물은 0.12.1 과 동일하다.

### 소비 프로젝트 계약이 **동작 계약**임을 밝힌다

README 「소비 프로젝트 계약」의 표를 **이름 목록으로 읽으면 틀린다.** `formControlsImport` 가
가리키는 모듈이 `UInput`·`USelect` 를 그 이름으로 export 하더라도, `onChange` 로 **값이 아니라
이벤트 객체를 넘기면** 계약은 충족되지 않는다. 웹 컴포넌트를 통과형으로 감싼 React 바인딩이
대표적으로 이 형태이며, TS strict 에서는 컴파일이 깨지고 느슨한 설정에서는 **이벤트 객체가 폼
모델에 저장된다.**

그 경우 필요한 것은 이름을 바꾸는 재수출이 아니라 **어댑터**(이벤트→값 변환, 배열 `options`→
자식 요소 투영, 프롭 이름 번역)다. 계약 절이 이제 그 사실과 판별 기준을 함께 적는다.

세 절의 소제목도 **경로가 아니라 옵션**을 앞세운다 — `#### formControlsImport 가 가리킬 모듈
(기본값 …)`. 종전 표기는 계약이 기본 경로에만 적용되는 것처럼 읽혔다.

그리고 **그 계약이 정확한 채로 유지되게** 했다. 방출되는 프롭이 계약 절에 없으면 테스트가
깨진다 — `step`·`maxlength` 가 0.8.0 에서 표에 반영된 것은 누가 기억했기 때문이었고, 이제
기억이 필요 없다. 계약을 테스트에 복제하지 않고 **README 를 읽는다**: 복제하면 두 곳이 어긋나고
그때도 알아차리는 것은 소비자다.

### 0.11.0 의 네임스페이스 안내가 **어느 버전부터인지** 말한다

0.11.0 은 생성 코드에서 완전수식명을 걷어내며 *"어떤 줄이 필요한지는 런타임 패키지의 README 가
안내한다"* 고 적었다. **그 안내가 가리킨 절은 런타임 패키지 0.10.0 에서 추가됐다** — 더 낮은
버전을 참조 중인 소비자는 지시를 정확히 따라가도 **동봉된 README 에서 그 절을 찾지 못한다.**
해당 항목이 이제 최소 버전과 절 이름을 함께 밝힌다.

## 0.12.1

> **0.11.1 은 이 문서 변경을 0.11.0 위에 얹은 패치다.** 게시 순서가 0.12.0 뒤가 됐으나
> 0.11 라인의 정상 패치이며, 0.12 를 쓰는 소비자는 이 릴리스를 받으면 된다.

### 릴리스 노트가 nuget.org 에서도 보인다

`PackageReleaseNotes` 가 비어 있었다. CHANGELOG 는 0.8.0부터 패키지에 동봉돼 왔지만,
nuget.org 가 「Release Notes」라는 이름으로 노출하는 자리는 빈 채였다 — **소비자는 그 이름을
찾고, 없으면 릴리스 노트가 없다고 읽는다.** 그 자리가 이제 CHANGELOG 를 지시한다.

머리말에 **건너뛴 구간을 전부 읽으라는 안내**도 넣었다. 각 릴리스는 자기 breaking 만 적으므로
최신 항목 하나만 읽으면 건너뛴 구간의 동작 변화가 그대로 남는다.

### 동봉 명세 사본이 생성기의 실제 동작을 말한다

`docs/M3L.md` §2.3.1.1 이 의미 타입 축약을 언어 명세 그대로만 적고 있었다. 그래서 두 가지가
어긋나 있었다 — `money` 의 확장형이 명세와 다른 값(`decimal(10,2)`)이었고, **이 생성기가
그 표를 그대로 구현하지 않는다는 사실이 어디에도 없었다.**

이제 그 절이 세 가지를 밝힌다: 상한과 형식 검증이 한 쌍이며 `phone` 만 정규화 후 형태의
상한이라는 것, 이 생성기의 `phone` 이 30(명세 20)이며 **축소는 저장 데이터를 보지 못하는
생성기가 대신 결정할 수 없다**는 것, 그리고 `money`·`percentage` 는 미구현이라 선언 시
빌드가 실패한다는 것. 형식 검증은 어느 타입에도 방출되지 않는다.

## 0.12.0

### 생성 폼이 소비자의 폴더 구조를 더 이상 강제하지 않는다

생성된 `{Entity}Form_gen.tsx` 는 컴포넌트·레이아웃·헬퍼 모듈을 **고정된 경로 문자열**로
임포트했다. 생성물은 `DO NOT EDIT` 이므로 그 경로는 소비자가 고칠 수 있는 기본값이 아니라
**소비자가 반드시 만들어야 하는 폴더 배치**였다 — 이미 쓸 만한 컴포넌트 라이브러리를 가진
프로젝트도 그 경로에 재수출 파일을 두어야 했고, 패키지 경계를 넘는 모노리포 배치에서는
상대경로가 성립하지 않았다.

세 개가 설정으로 열렸다(TypeScript 타깃):

| 키 | 기본값 | 공급하는 것 |
|---|---|---|
| `formLayoutImport` | `@iyulab/enterprise` | `FormSection` · `FormRow` |
| `formControlsImport` | `../components/ui` | `UInput` · `UTextarea` · `USelect` · `UCheckbox` |
| `formSelectOptionsImport` | `../lib/select-options` | `enumToOptions` |

**기본값은 종전 문자열 그대로다.** 설정을 추가하지 않은 프로젝트의 생성물은 변하지 않는다.
기본값은 추천이 아니라 **호환을 위해 유지되는 값**이며, 어느 라이브러리를 가리킬지는
소비자 결정이다. 계약은 여전히 *"이 export 들이 필요하다"* 까지이고 *"어디에 두라"* 가 아니다.

### 🔴 생성 폼이 생성 타입을 못 찾을 수 있었다 (버그)

`outputPath` 와 `formsOutputPath` 는 서로 독립적인 설정인데, 폼은 생성 타입을
`'../types/entities_gen'` 라는 고정 문자열로 임포트했다. 즉 그 임포트는 *"폼 디렉터리의
부모 아래에 `types` 라는 이름으로 타입 디렉터리가 있다"* 가 참일 때만 해소된다. 그 관계를
검증하는 곳은 없었다.

어긋난 조합에서는 **생성이 성공을 보고하고, 실패는 소비 프로젝트의 컴파일 단계에서**
드러났다. 이제 이 경로는 두 설정에서 **계산**된다 — 어떤 조합이든 폼은 생성된 타입을 찾는다.
상대경로가 성립하지 않는 조합(서로 다른 드라이브·루트)에서는 **생성이 실패**하며, 컴파일되지
않는 파일을 남기지 않는다.

생성 타입 경로는 **설정 항목이 아니다.** 도구가 이미 아는 값을 되묻는 옵션은 어긋날 수 있는
정본을 하나 더 만들 뿐이다.

### 재발 방지

새로 추가되는 임포트가 모듈을 문자열로 지목하면 테스트가 깨진다. 방출된 모든 모듈 지정자는
**설정에서 왔거나, 출력 경로에서 계산됐거나, 이유가 기록된 예외**여야 한다(현재 예외는
`react` 하나 — React 컴포넌트가 React 를 구성 가능한 위치에서 가져올 수는 없다).

### 파괴적 변경 없음

설정을 추가하지 않으면 산출물이 동일하다. 소비자 조치는 없다.

---

## 0.11.0

### 타입이 함의하는 길이 상한이 모든 타깃에 도달한다

`email`·`phone`·`url` 은 선언에 `(n)` 을 적지 않아도 상한을 가진다 — 언어 명세 §10.4.2가
확장형을 규정한다. 지금까지 그 상한은 **SQL 컬럼에만** 있었다. 판별자가 명시된 `string(n)` 만
읽었기 때문이고, 그래서 0.10.0이 `string(n)` 에 대해 닫은 결함이 이 타입들에는 그대로 남아
있었다 — 상한을 넘는 값이 API 검증을 통과해 데이터베이스에서 provider 원본 오류로 되돌아왔다.

이제 컬럼·C# 엔티티(`[StringLength]`)·필드 스키마(`maxLength`)·생성 폼(`maxlength`) **넷이
같은 값**을 쓴다. 네 산출물의 합의는 테스트로 고정된다.

### ⚠️ 상한 값이 바뀐다 — 명세와 어긋나 있었다

| 타입 | 명세 §10.4.2 | 0.10.0 | 0.11.0 |
|---|---|---|---|
| `email` | `string(320)` | 200 | **320** |
| `url` | `string(2048)` | 500 | **2048** |
| `phone` | `string(20)` | 30 | 30 (유지) |

**세 값 모두 명세와 달랐다.** 파서는 의미 타입을 확장하지 않고 이름 그대로 보고하므로,
확장 값은 각 소비자가 표로 들고 있어야 하고 그 표가 어긋나 있었다.

`email`·`url` 은 **넓어진다.** 생성 스키마가 바뀌므로 적용 시 컬럼 확장이 따라오며,
데이터 손실은 없다 — 오히려 지금까지 컬럼에서 거부되던 201~320자 값이 통과하게 된다.

`phone` 은 **30을 유지한다.** 명세를 따르면 축소이고, 이미 저장된 값이 새 상한을 넘으면
마이그레이션이 실패하거나 값이 잘린다. 생성기는 저장된 데이터를 볼 수 없으므로 그 결정을
대신 내리지 않는다. 이탈은 코드에 사유와 함께 기록돼 있고, 명세보다 **넓은** 방향이므로
명세가 허용하는 값이 거부되는 일은 없다.

### 업그레이드 시 확인할 것

- **`FieldSchema.maxLength` 에 새 항목이 나타난다.** 스키마를 읽어 자체 검증을 도는 클라이언트
  코드가 영향받는 유일한 표면이다.
- **폼의 `maxlength`** 는 저장된 값을 무효화하지 못한다 — 상한이 넓어지는 방향이라 더욱 그렇다.
- **형식 검증은 포함되지 않는다.** 321자 이메일은 거부되지만 `not-an-email` 은 그대로 통과한다.
  형식은 여전히 애플리케이션 계층의 몫이다.

### ⚠️ 생성 코드가 네임스페이스를 적지 않는다 — 소비 프로젝트 조치 필요

생성된 C# 이 지금까지 런타임 타입들을 완전수식명(`global::…`)으로 적었다. 이제 **단순명**만
적고, 이름을 어느 네임스페이스에서 찾을지는 소비 프로젝트가 정한다.

이 생성기가 남의 패키지 배치를 알고 있을 이유가 없다. 알고 있으면 그쪽에서 네임스페이스 하나만
옮겨도 이쪽 생성물이 깨지고, 고칠 곳은 생성기가 된다. 이름만 적으면 그 결합이 사라진다.

**조치** — 소비 프로젝트에 `global using` 을 선언한다. 어떤 줄이 필요한지는 **런타임 패키지
0.10.0 이상**의 README 「Namespaces a consumer needs」절이 안내한다 — 그 절은 0.10.0 에서
추가됐으므로, **더 낮은 버전을 참조 중이라면 동봉된 README 에는 그 안내가 없다.** 먼저 올릴 것.
누락 시 나오는 오류는 타입명을 지목하는 평범한 `CS0246` 이다.

> 필요한 네임스페이스를 여기 옮겨 적지 않는 것은 의도적이다. 이 생성기가 남의 패키지 배치를
> 아는 것이 위 문단이 제거한 결합이며, **그 지식을 코드 대신 문서에 적어도 같은 결합이다.**
> 정본은 그 패키지의 README 한 곳이고, 그래야 네임스페이스가 바뀔 때 고칠 자리도 한 곳이다.

`@implements` / `@inherits` 로 **모델이 직접 적은 완전수식명**은 그대로 유지된다 —
그 이름은 모델 작성자가 고른 것이지 생성기가 고른 것이 아니다.

### 같은 결함이 다시 숨지 못하게 하는 것

이 결함이 오래 보이지 않은 이유는 값이 틀려서가 아니라 **검사가 그 축을 보지 못해서**다.
세 가지를 함께 고쳤다.

- **길이 상한의 정본이 하나가 됐다.** 두 방언 매퍼가 각자 들고 있던 사본을 없앴다.
- **컬럼 대조 검사가 문자열 아닌 상한도 본다.** 어느 분류에도 들어가지 않는 상한 있는 컬럼은
  이제 판단을 적기 전까지 검사를 통과하지 못한다. `binary(n)` 의 상한이 엔티티에 도달하지
  않는다는 사실은 **기록으로 단언**된다 — 검사에서 제외하면 그 축에 대해 다시 침묵하게 된다.
- **타입 어휘에 처분 표가 생겼다.** 선언 어휘에만 있던 래칫의 짝이다. 타입에 암묵 상한을
  주면서 엔티티로 나르지 않는 것이 이제 불가능하다.
- **생성물에 생성기가 스스로 붙인 완전수식명이 남지 않는다**는 것을 전체 파이프라인 산출물
  대상으로 검사한다. 모델이 적은 완전수식명과 BCL 참조만 예외다.

## 0.10.0

### 선언된 제약이 C# 엔티티까지 도달한다

모델이 선언한 널 허용 여부·길이 상한·기본값은 지금까지 SQL 컬럼과 생성 폼에는 반영됐지만
**C# 엔티티에는 반영되지 않았다.** 그래서 API 표면이 모델이 금지한 값을 받아들였고, 위반은
데이터베이스까지 내려가 provider 원본 오류로 되돌아왔다. 세 축을 모두 방출한다:

| 선언 | 방출 |
|---|---|
| 널 허용하지 않는 참조형 필드(`string`/`text`/`json`/`phone`/`email`/`url`/`binary`) | `[Required]` |
| `string(n)` | `[StringLength(n)]` — 널 허용 여부와 무관 |
| `= <value>` | C# 속성 초기화자 (`= true;` · `= 3;` · `= "NEW";` · `= 0.5m;` · `= Status.Draft;`) |

`@not_null` 을 적었는지가 아니라 **필드가 실제로 널을 허용하는지**가 기준이다 —
`- name: string(50)` 처럼 속성 없이 선언한 필드도 컬럼이 `NOT NULL` 이므로 함께 방출된다.

파생 필드(`@lookup`/`@rollup`/`@computed`)에는 `[Required]` 가 붙지 않는다. 뷰가 채우는 값이라
호출자가 보낼 수 없고, 붙이면 생성 요청이 전부 거부된다.

기본값은 **C# 리터럴로 표현할 수 있는 타입에만** 방출된다. 시간 타입·`identifier`·`binary` 는
기본값이 무엇이든 초기화자를 만들지 않으므로, `= now()` 같은 서버 측 기본값이 코드로 새지 않는다.

### ⚠️ 소비자 조치 필요 — 지금까지 통과하던 요청 일부가 400 이 된다

두 가지 이유로 동작이 바뀐다. 둘 다 **모델이 이미 선언하고 있던 것**을 집행하는 것이지만,
그동안 조용히 통과하던 호출자가 있었다면 업그레이드 시 드러난다.

**1. 빈 문자열이 거부된다 — `NOT NULL` 보다 좁다.**
SQL `NOT NULL` 컬럼은 빈 문자열 `''` 를 **허용**하지만 `[Required]` 는 거부하며, 판정 전에
문자열을 trim 하므로 **공백만 있는 값도 거부**한다. 즉 널 허용하지 않는 문자열 필드는 이제
API 표면에서 "값이 있어야 한다"를 뜻한다. 빈 문자열을 정상 값으로 저장하고 있었다면 그 호출은
실패하게 된다.

**2. 길이 초과가 데이터베이스에 닿기 전에 거부된다.** 이전에는 컬럼에서 잘리거나 실패했다.

부분 수정(PATCH)은 영향받지 않는다 — 델타 바인딩은 보내지 않은 속성을 검증 대상으로 삼지 않으므로,
필수 필드를 포함하지 않는 부분 수정은 그대로 성공한다.

선언된 기본값을 가진 필드는 **초기화자가 함께 방출되므로** 그 필드를 생략한 요청이 새로
거부되는 일은 없다. 다만 그동안 CLR 기본값(`false`/`0`)이 저장되던 필드에는 이제 **선언한 값이**
저장된다 — `boolean = true` 로 선언한 필드가 생성 시 `false` 로 저장되던 동작이 고쳐진 것이다.

### 내부 — 타깃 간 제약 처분 래칫

선언 어휘 전체에 대해 **Model 타깃이 그것을 어떻게 처리하는지**를 기록하고, 새 어휘가 추가되면
처분이 기록될 때까지 테스트가 실패한다. 한 타깃에만 반영하고 다른 타깃에서 잊는 일이 아무 신호
없이 지나가던 것을 막는다. 현재 기록된 타깃 간 비대칭: `@unique` · `@index` · `@min` · `@max` · `@help`.

### 내부 — acceptance 게이트가 어디서나 돈다

기능 행렬을 한 번에 교차하는 대규모 모델(24 엔티티 + 15 enum)이 리포에 들어왔다. 게이트는 그
모델로 4-타깃 빌드를 돌린 뒤 산출물을 **선언과 이름 단위로 대조**한다 — 기대값을 파싱한 모델에서
도출하므로 모델을 키워도 테스트의 숫자를 고칠 일이 없고, 산출물이 빠진 선언은 개수 차이가 아니라
이름으로 보고된다.

이전 게이트는 리포 밖 경로를 참조했고 그 경로가 없으면 **조용히 통과**했다 — 경로를 가진 한
대에서만 실제로 실행되고 나머지 모든 실행에서는 아무것도 검증하지 않으면서 통과 수에는 포함됐다.
`MDDBOOSTER_ACCEPTANCE_MODEL` 로 다른 모델을 지정할 수 있지만, 지정한 경로가 없으면 이제
**건너뛰지 않고 실패**한다.

### 외래 키 인덱스 opt-in — `emitForeignKeyIndexes` (Sql 타깃, 기본 `false`)

두 엔진 모두 외래 키를 자동 인덱싱하지 않는다. 그래서 FK 로 하는 조인과, 삭제 시 참조 행을
확인하는 검사가 자식 테이블을 훑는다. 켜면 **모델이 덮지 않은 FK 컬럼마다** 인덱스를 만든다.

```json
{ "type": "Sql", "projectPath": "../db", "emitForeignKeyIndexes": true }
```

- **방언 중립** — T-SQL(`IX_{Model}_{Column}`)과 PostgreSQL(`ix_{table}_{column}`) 양쪽.
  어느 컬럼이 대상인지는 한 판정기가 정하므로 두 방언이 어긋날 수 없다.
  (한 가지 예외를 알고 있다 — 널 허용 유니크 컬럼은 T-SQL 에서 filtered index 가 되어 일반
  조인에는 쓰이지 않지만 판정기는 양쪽 모두 덮인 것으로 본다. 좁은 형태이고, 고치려면 판정기가
  방언을 알아야 해서 유지했다. 코드 주석에 기재.)
- **이미 덮인 컬럼은 건너뛴다** — `@pk`·`@unique`·`@index` 는 물론, **복합 인덱스·복합 유니크의
  선두 컬럼**도 대상이 아니다. `(a, b)` 인덱스는 `a` 만으로 하는 조회도 처리하므로 `a` 에
  인덱스를 또 만들면 쓰기 비용만 늘어난다.
- **기본값은 `false`** — 켜는 것은 읽기 이득과 쓰기·저장 비용의 교환이고, 기존 스키마에 조용히
  적용할 판단이 아니다. 끈 상태의 산출물은 이전과 같다.

> 24 엔티티 모델 실측: FK 제약 28개 · 모델이 선언한 인덱스 13개 · 옵션을 켜서 늘어난 인덱스
> **9개**(22개). 선두 컬럼 인지가 없었다면 이미 덮인 컬럼에 인덱스가 더 붙었을 것이다.

### PostgreSQL — `@index` 선언이 인덱스로 방출된다

PG 렌더러는 `@index`(필드 선언과 `### Indexes` 섹션 양쪽)를 **방출하지 않고 경고**했다. 근거는
선언형 적용 도구가 인덱스를 지원하지 않는다는 것이었고, 그 조건이 해소됐다.

```sql
CREATE TABLE public.facility_inspection ( … );

CREATE INDEX ix_facility_inspection_status ON public.facility_inspection (status);
```

- 인덱스는 `CREATE TABLE` **뒤의 독립 문**으로 나간다.
- PK·UNIQUE 제약이 소유하는 인덱스는 중복 방출하지 않는다 — 선언 상태 비교에서 제약이 소유한
  인덱스는 제외되므로, 중복을 내면 매 적용마다 살아남는다.
- `CONCURRENTLY` 는 쓰지 않는다 — 적용이 단일 트랜잭션이고 concurrent 빌드는 그 안에서 실행될
  수 없다.
- 인덱스명(`ix_{table}_{cols}`)도 제약명과 같은 63바이트 게이트를 통과해야 한다. 다중 컬럼
  인덱스명은 제약명보다 쉽게 넘기며, 넘기면 조용히 절단되는 대신 빌드가 실패한다.

> **선언이 없으면 산출물은 바뀌지 않는다.** 이 변경은 `@index` 를 적은 모델에만 효과가 있다.
> FK 컬럼에 인덱스를 자동으로 만드는 정책은 **별개**이며 아직 없다.

### 내부 — snake_case → PascalCase 변환을 한 곳으로

같은 변환의 손구현 사본이 네 생성기에 걸쳐 **12벌** 있었다(동작은 전부 같았고, 하나만 빈 문자열
가드가 없었다). `NameCasing.ToPascalCase` 로 모으고 사본을 제거했다. 산출물은 네 타깃 전부
바이트 단위로 동일하다.

정리 문제가 아니라 정확성 문제가 됐기 때문이다 — enum 멤버명은 **enum 선언에서 한 번, 그 멤버를
참조하는 속성 초기화자에서 다시 한 번** 올려지고 둘은 서로 다른 렌더러가 낸다. 두 변환이
어긋나면 존재하지 않는 멤버를 가리키는, **컴파일되지 않는 초기화자**가 나온다.

### 문서 정정

`@unique(col1, col2)` 복합과 `### Indexes` 섹션이 기능 표에 미구현(⏳)으로 남아 있었으나
**둘 다 구현돼 있다.** 빌드 실행 예시도 동작하지 않는 형태였다(`mdd <dir>` → `build` 가 필수).

### 설치 스크립트 기본 경로

`scripts/install.ps1` 의 기본 설치 경로가 `%LOCALAPPDATA%\Programs\mdd-booster` 가 됐다
(이전에는 특정 드라이브 문자를 가정했다). `-InstallPath` 로 계속 지정할 수 있다.

---

## 0.9.0

### ⚠️ 소비자 조치 필요 — 이름이 바뀐다 (대문자 약어로 시작하는 엔티티가 있는 경우)

엔티티명이 **대문자 약어로 시작하면**(`QRScanLog`, `QRCode`) 지금까지 camelCase 변환이
첫 글자만 소문자화해 어색한 이름을 만들었다. `.NET JsonNamingPolicy.CamelCase`와 같은
표준 규칙으로 정정했다:

| 표면 | before | after |
|---|---|---|
| GraphQL query / mutation | `("qRScanLogs", "qRScanLog")` | **`("qrScanLogs", "qrScanLog")`** |
| 생성 폼 헬퍼 (`*Form_gen.tsx`) | `export function qRScanLogFromEntity` | **`export function qrScanLogFromEntity`** |
| OData entity set | `"QRScanLogs"` | `"QRScanLogs"` — **변경 없음** (DbSet 이름과 맞춤) |

후행 약어는 보존한다: `OrderQR` → `orderQR`.

**재생성만으로 회수되지 않는 곳이 하나 있다 — 손으로 쓴 `AddEntityPair` 호출부.**
생성물 밖에서 `options.GraphQL.AddEntityPair<…>("qRScanLogs", "qRScanLog")` 를 직접 적어 둔
컴포지션 코드가 있으면 **컴파일 오류가 나지 않는다** — 그 서버만 옛 필드명을 계속 노출하고,
재생성된 다른 서버는 새 필드명을 노출해 **같은 엔티티가 서버마다 다른 이름으로 보인다**.
업그레이드 시 `AddEntityPair(` 를 검색해 손으로 적은 인자를 함께 고칠 것.

기존 GraphQL 쿼리가 옛 필드명을 참조하고 있으면 그 쿼리도 함께 갱신해야 한다.
(영향 판정 방법: 모델 정본에서 `^[A-Z]{2,}` 로 시작하는 엔티티가 없으면 이 릴리스는 무영향이다.)

### ⚠️ 소비자 조치 필요 — `EntitySetName` union 이 좁아진다 (`@internal` 모델이 있는 경우)

`entity_names_gen.ts` 의 `ENTITY_NAMES`/`EntitySetName` 에서 **`@internal` 모델이 빠진다.**

이 목록은 OData entity set 이름의 미러이고 소비자가 이걸로 OData URL 을 만든다. 그런데
`@internal` 모델은 `AddEntityPair` 가 등록하지 않으므로 **그 이름들은 존재하지 않는 경로였다** —
타입세이프하게 404 를 광고하고 있었던 셈이다. 결함 수정이다.

`EntitySetName` 을 좁히므로, 그 이름을 쓰던 코드가 있으면 컴파일 오류가 난다(그 경로는
애초에 동작하지 않았다). **`entities_gen.ts` 의 인터페이스·`*Form_gen.tsx` 는 그대로 유지된다** —
타입은 데이터 API 전용이 아니고 전용 엔드포인트로 관리되는 엔티티에도 유용하기 때문이다.

### 추가 — 타깃별 엔티티 부분집합 (`includeEntities` / `excludeEntities`)

한 모델 정본을 여러 서버가 공유할 때, **표면 타깃(Api·TypeScript)** 이 노출할 엔티티를
타깃별로 좁힐 수 있다. 둘 다 생략하면 전량 — **완전 하위호환**이다.

```json
{ "type": "Api", "projectPath": "../src/MyApp.MesServer", "namespace": "MyApp.MesServer",
  "includeEntities": ["ProductionWork", "QRScanLog"] }
```

- 둘을 함께 지정 · 없는 엔티티명(가장 가까운 이름 제안 포함) · `includeEntities` 에 `@internal`
  모델 · `Sql`/`Model` 타깃에 지정 — **전부 빌드 오류**다. 조용히 무시하거나 부분 적용하지 않는다.
- 필터가 걸린 타깃은 빌드마다 커버리지를 출력한다(`포함 N개 / 제외 M개 — 이름…`).
- ⚠️ `includeEntities` 는 drift 한다 — 정본의 새 엔티티가 이 타깃에는 조용히 안 나타난다.
  신규 엔티티 기본 노출을 원하면 `excludeEntities` 를 쓸 것.

- **Api 타깃만 좁히면 경고**한다. `EntitySetName` 은 OData entity set 이름의 미러이므로 Api 표면만
  좁히면 어떤 서버도 등록하지 않는 이름을 계속 광고한다. 같은 필터를 TypeScript 타깃에도 지정하면
  사라진다. 판정은 전 Api 타깃의 합집합 기준이라 공유 UI 가 여러 서버를 담당하는 구성은 대상이 아니다.

전체 규칙표는 README "타깃별 엔티티 부분집합" 절 참조.

### 수정 — 복수 타깃의 조용한 오출력 2건이 빌드 오류로 올라갔다

한 정본을 여러 서버가 소비하는 구성에서 **빌드는 성공하는데 산출물이 틀린** 두 경우가 있었다.
둘 다 소비자가 컴파일 오류나 죽은 라우트를 만난 뒤에야 원인을 찾게 되는 형태였다.

- **`Model` 타깃이 2개 이상일 때** `Api` 타깃이 **첫 번째** Model 의 namespace 를 집고 나머지를
  조용히 무시했다 → 잘못된 `using` 방출. 이제 후보가 둘 이상이면 추론하지 않고 빌드 오류로
  **`entitiesNamespace` 명시를 요구**한다. 신규 옵션 `entitiesNamespace` (Api 타깃, 생략 시 종전대로 추론).
  Model 타깃이 하나인 기존 구성은 **무영향**이다.
- **같은 종류·같은 경로 타깃이 2개**면 나중 것이 앞선 것의 산출물을 조용히 덮어썼다 → 빌드 오류.
  경로는 정규화해 비교하므로 `../api` 와 `../x/../api` 도 중복으로 잡힌다.
  같은 종류라도 **경로가 다르면 정상**이다(복수 서버 시나리오).

### 내부 — 단어분리 규칙이 한 곳으로 모였다

PascalCase 단어분리(ADR-0001 §2.1)가 snake_case 경로에만 있고 camelCase 경로 두 곳은
각자 손구현 사본을 갖고 있어서 `QRScanLog`가 `qr_scan_log`(정상)와 `qRScanLog`(훼손)로
갈라졌다. `MddBooster.Core.Naming.NameCasing`이 분리기의 정본이 되고 두 사본을 제거했다.
GraphQL 복수형은 이제 **camelCase를 먼저 적용한 뒤 복수화**한다 — 순서가 반대면 두 글자
약어가 `QR` → `QRs` → `qRs`로 깨진다.

---

## 0.8.0

### ⚠️ 소비자 조치 필요 (TypeScript 타깃을 쓰는 경우)

생성 폼(`*Form_gen.tsx`)이 **새 프롭 2종을 방출**한다. 소비 프로젝트의
`../components/ui` 배럴에 있는 `UInput` 래퍼가 이들을 받아 아래 input으로 전달해야 하며,
**없으면 재생성 후 `tsc`가 TS2322로 실패한다.**

```ts
export function UInput(props: {
  label: string
  value: string
  onChange: (v: string) => void
  required?: boolean
  type?: string
  description?: string
  step?: number        // ← 0.8.0 신규. decimal 필드가 있는 모델
  maxlength?: number   // ← 0.8.0 신규. string(n) 필드가 있는 모델
}) { /* 아래 input으로 그대로 전달 */ }
```

- **`maxlength`가 더 넓게 영향한다.** `decimal`은 안 쓸 수 있어도 `string(n)`은 거의 모든
  모델에 있다. `decimal`을 쓰지 않는 소비자도 이 프롭은 거의 확실히 필요하다.
- 표기 주의: **소문자 `maxlength`** (React DOM의 `maxLength` 아님), 두 값 모두 **`number`**
  (생성물은 `step={0.0001}` · `maxlength={50}` 형태의 중괄호 숫자 리터럴을 방출한다).
- `description?`은 0.8.0 신규가 아니라 **기존 계약**이다. 래퍼에 없다면 어떤 필드든
  `@help("...")`를 붙이는 순간 같은 방식으로 깨진다 — 이번 기회에 함께 갖출 것.

전체 계약은 README의 "소비 프로젝트 계약 (TypeScript 타깃)" 절 참조.

### 추가 — 모델 타입 정보가 입력 어포던스로 전달된다

생성 폼이 모델의 타입 정보를 버리고 있던 지점을 메웠다. SQL 타깃(`DECIMAL(p,s)`,
`NVARCHAR(n)`)과 Model 타깃(`[Column(TypeName)]`)은 같은 정보를 이미 소비하고 있었고,
TypeScript 폼 타깃만 버리고 있었다.

- **`decimal(p,s)` → `step={10^-s}`** — `decimal(18,4)`는 `step={0.0001}`.
  이전에는 `step`이 없어 브라우저 기본값 1이 적용됐고, **소수 입력이 필요한 필드는 저장이
  불가능**했다("Value must be a multiple of 1"). 오류 없이 submit이 조용히 막히는 형태라
  앱에는 아무 신호도 가지 않았다.
  - 파라미터 없는 `decimal`은 `step={0.01}` (SQL 기본값 `DECIMAL(18,2)`와 일치).
  - 정수 타입과 `decimal(p,0)`은 `step`을 방출하지 않는다 — HTML 기본값 1이 정확히 맞다.
- **`string(n)` → `maxlength={n}`** — `NVARCHAR(n)`의 상한을 UI로 앞당긴다.
  저장된 값은 이미 n 이내이므로 기존 값을 무효화하지 않는다. 상한이 없는 무파라미터 `string`과
  `text`(둘 다 `NVARCHAR(MAX)`)는 방출하지 않는다.

### 알려진 한계 (의도적 미구현 — 문서화)

- **`float`/`double`은 여전히 소수 입력이 막힌다.** 스케일 개념이 없어 `step="any"`가 유일한
  정답인데, 소비자 계약상 `step`이 `number`라 `"any"`를 실을 수 없다.
  **소수가 필요한 필드는 `float`/`double` 대신 `decimal(p,s)`로 모델링할 것** —
  정밀도가 명시되므로 SQL·EF·폼이 모두 같은 약속을 하게 된다.
- **`timestamp`/`datetime`/`time`은 자유 텍스트 입력으로 남는다 — 이것은 결정이다.**
  `DateTimeOffset`이 직렬화하는 `"2026-07-28T14:30:00+09:00"`을 `<input type="datetime-local">`이
  받지 못하고(오프셋 불가), `TimeOnly`의 소수 초를 `<input type="time">`이 받지 못한다
  (기본 `step=60`, 소수 3자리 한계). 피커를 붙이면 브라우저가 값을 거부해 컨트롤이 **빈 칸**이
  되고 그대로 저장하면 **기존 값이 지워진다** — 값을 보여주기라도 하는 자유 텍스트보다 나쁘다.
  `date`만 피커를 받는 이유도 이것이다(`DateOnly` → `"2026-07-28"`은 컨트롤이 그대로 받는다).

### 내부

- `FieldAttributes.TypeParams` / `FieldAttributes.StringMaxLength`를 `MddBooster.Core`의
  정본으로 세우고, `EntityPairRenderer`·`TsFieldSchemaRenderer`가 각자 갖고 있던 동등 로직을
  제거했다. 같은 컬럼의 정밀도·길이를 두 곳이 각자 계산하던 상태를 해소.


---

## 0.5.0

> **소급 기재** — 이 절은 릴리스 전체가 아니라 **생성기가 해석하는 attribute 가 어느
> 버전부터 쓸 수 있는지**만 적는다. 도출 방법은 문서 머리말 참조.

### enum 값의 `@system` — 생성 폼 선택지에서 제외

**`v0.5.0` 부터 쓸 수 있다.**

enum **값**에 붙인다. 표시 라벨·C# enum·SQL CHECK 제약에는 그대로 남고, 생성 폼의
**입력 선택지에서만** 빠진다 — 저장을 제한하는 것이 아니라 작성을 제한한다.

```markdown
## PaymentMethod ::enum
- card: "카드"
- legacy_carryover: "레거시 이관 정리" @system
```

전체 계약은 README 「enum 값의 `@system`·`@deprecated`」 절 참조.
이후 변경: `0.23.0` 이 같은 규칙에 값 레벨 `@deprecated` 를 더했고, 제외된 값을 든
기존 행을 편집할 때 그 값이 선택지에서 사라지던 결함을 고쳤다.

---

## 0.3.0

> **소급 기재** — 위와 같다.
>
> `v0.3.0`(2026-07-13)은 **현재 라인의 첫 릴리스**다. 아래 넷은 그 전부터 소스에
> 있었으므로 「0.3.0 에서 추가됐다」가 아니라 **「0.3.0 부터 배포본으로 쓸 수 있다」**가
> 정확한 서술이다.

### 폼 생성 attribute 4종 — `@group` · `@help` · `@slot` · `@display_labels`

**`v0.3.0` 부터 쓸 수 있다.**

| attribute | 대상 | 생성 결과 |
|---|---|---|
| `@group("이름")` | 필드 | 폼을 `<FormSection title="이름">`으로 묶는다. 없으면 `"기타"` 섹션 |
| `@help("설명")` | 필드 | 컨트롤에 `description="설명"` — 라벨은 짧게 두고 예시·부연을 아래로 분리 |
| `@slot` | 필드 | 인라인 컨트롤 대신 **슬롯 자리표시자**로 렌더 (호출부가 내용을 주입). `@reference` FK 필드는 자동으로 슬롯 |
| `@display_labels(다른Enum)` | enum 필드 | 표시 텍스트만 다른 enum의 라벨맵으로 교체. **저장/캐스트 타입은 그대로** |

전체 계약은 README 「생성기 해석 attribute (TypeScript 타깃)」 절 참조.

⚠ `@help` 는 소비 프로젝트의 컨트롤 래퍼가 `description?` prop 을 갖추고 있을 것을
요구한다 — 없는 래퍼에서는 어떤 필드든 `@help` 를 붙이는 순간 빌드가 깨진다
(`0.8.0` 「소비 프로젝트 계약」 절 참조).
