...
전량 쓰는데 폼은 일부만 필요하다」*는 구성에는 쓸 수 없다 — 좁히는 순간 `entities_gen.ts` 의
타입도 함께 사라지기 때문이다.

**엔티티 수 > 화면 수인 앱**이 그 자리에 온다. 접합 테이블·감사 로그·부모 화면에 인라인으로
편집되는 자식은 단독 폼을 가질 이유가 없는데, 목록·표에서는 그 타입을 계속 쓴다.
...
| 규칙 | 동작 |
|---|---|
| 둘 다 생략 | 타깃 범위 전량에 폼 (현행) |
| `formsInclude` / `formsExclude` | 화이트리스트 / 블랙리스트 — 위와 같은 술어 |
| 둘 다 지정 · 없는 엔티티명 | **빌드 오류** (위와 동일) |
| `formsOutputPath` 없이 지정 | **빌드 오류** — 폼을 아예 안 내는 설정이라 필터가 무의미하다 |
| `TypeScript` 아닌 타깃에 지정 | **빌드 오류** — 폼을 내는 타깃은 그것뿐이다 |
| 타깃 범위 **밖**의 엔티티를 `formsInclude` 에 | **빌드 오류** — 아래 |

🔴 **폼 집합은 언제나 타깃 집합의 부분집합이다.** 생성 폼은 자기 타입을 `entities_gen.ts` 에서
임포트하므로, `includeEntities` 가 이미 제외한 엔티티의 폼을 내면 **임포트가 깨진 파일**이
나온다. 그 조합은 조용히 넘기지 않고 오류로 세운다.
...
> ✅ **이 계약은 이제 이 저장소 안에서 기계적으로 검사된다.** 테스트 스위트가 아래 표면을
> 그대로 선언한 `.d.ts` 를 두고, 생성 폼 전체를 `tsc --strict` 로 컴파일한다 — 즉 여기 적힌
> 계약을 만족하는 모듈을 가리켰다면 생성 코드는 컴파일된다. **다만 검사되는 것은 «타입»이다**:
> 이름과 타입이 맞는데 동작이 다른 구현(아래 경고의 이벤트-대-값 사례)은 여전히 소비앱에서만
> 드러난다.
...
>
> ℹ️ **빌드가 이것을 먼저 말한다.** `time` 필드가 폼 타깃에 닿으면 `mdd build` 가 안내 한 줄을
> stdout 에 낸다(`[typescript] 안내: …`) — 어느 필드인지, 의도된 것이라는 것, 그리고 이 절을
> 가리킨다. 이 절을 찾아 읽기 전에 생성물만 보고 결함으로 판단하는 일을 막으려는 것이므로,
> 안내가 거슬린다면 그것은 **이미 답을 안다는 뜻**이다 — 진단이 아니라 stdout 이라
> `2>` 로 경고·오류만 보는 파이프라인에는 섞이지 않는다.
...
### 경고를 에러로 — `treatWarningsAsErrors` · `warningsAsErrors` · `noWarn`

경고는 기본적으로 표면화만 하고 빌드는 성공한다(종료 코드 0). «경고 0 빌드»를 CI 에 걸려면 콘솔 문구를 찾지 말고
`mdd.json` 최상위에 **코드로** 적는다 — 이름과 우선순위는 MSBuild 의 같은 이름 속성을 따른다:
...
| 키 | 의미 |
|---|---|
| `treatWarningsAsErrors` | 코드 있는 경고 전부를 에러로 |
| `warningsAsErrors` | 나열한 코드만 에러로 (예: `["MDD017", "MDD019"]`) |
| `noWarn` | 나열한 코드는 보고하지 않는다 — 두 승격 키보다 **우선**한다 |

- 대상은 **코드가 있는 경고**다: 이 생성기의 의미 분석(`MDDnnn`)과 m3l 파서·검증기(`M3L-Wnnn`). 코드 없는 알림
  (`[config] 경고` · 소비되지 않는 요소 회계)은 대상이 아니다.
- 승격된 경고는 `에러(경고 승격)` 으로 찍히고, m3l·의미 분석 두 층을 다 모은 뒤 **생성 전에** 종료 코드 3 으로 선다.
- 목록에 코드 형식이 아닌 값이 있거나 같은 코드가 두 목록에 함께 있으면 **설정 오류**(종료 코드 4)다 — 잘못 적힌 코드는
  조용히 발화하지 않는 규칙이 되기 때문이다.

### 소비되지 않는 요소 회계

**파싱된 모든 요소는 소비되거나 경고된다.** 어떤 타깃도 산출물을 만들지 않는 요소가 모델에
들어오면 빌드가 그것을 열거한다 — 조용히 사라지면 소비자는 산출물이 없는 이유를 알 수 없다.

```
[m3l] 경고: 소비되지 않는 요소 3개 — 어떤 타깃도 산출물을 생성하지 않습니다: Article: ### Relations, Article: ### Behaviors, Article: ### Metadata
```

대상: standalone `::view` · `::flow` · extension · **그리고 `### Indexes` 를 제외한 모든 `###`
섹션.** 섹션 중 타깃이 읽는 것은 `### Indexes` 하나뿐이다 — `### Relations`·`### Behaviors`·
`### Metadata` 와 언어가 이름조차 정의하지 않은 섹션(`### PrimaryKey` 등)은 전부 산출물에 흔적을
남기지 않는다(다섯 타깃 산출물 대조로 확인).

> ⚠️ **관계 선언은 `@reference` 로 한다.** M3L 은 `### Relations` 섹션도 제시하지만 이 생성기는
> 그 섹션도 `@relation(...)` 속성도 읽지 않는다 — 외래 키·내비게이션·lookup 리다이렉트는 전부
> `@reference(Target)` 에서 나온다.
