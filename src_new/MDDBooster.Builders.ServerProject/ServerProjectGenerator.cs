...
/// <summary>
/// Generator for GraphQL server classes
/// </summary>
public class ServerProjectGenerator
{
    ...
    public string GenerateFieldType(MDDModel model)
    {
        return ErrorHandling.ExecuteSafely(() =>
        {
            ...
            sb.AppendLine("            {");
            sb.AppendLine($"                Name = nameof({model.BaseModel.Name}._id)");
            sb.AppendLine("            }");
            sb.AppendLine("        });");
            sb.AppendLine();
            ...
            // Generate arguments based on model fields
            sb.AppendLine("        this.Arguments = new QueryArguments(new List<QueryArgument>");
            sb.AppendLine("        {");

            var allFields = ModelUtilities.GetAllFields(Document, model);
            var searchableFields = allFields.Where(f => !f.BaseField.IsPrimaryKey).ToList();
            ...
            foreach (var field in searchableFields)
            {
                ...
                sb.AppendLine("            {");
                sb.AppendLine($"                Name = nameof({model.BaseModel.Name}sSearchRequest.{propertyName}s),");
                sb.AppendLine("            },");
            }
            ...
            sb.AppendLine("            {");
            sb.AppendLine($"                Name = nameof({model.BaseModel.Name}sSearchRequest._ids),");
            sb.AppendLine("            },");
    ...
    public string GenerateRepository(MDDModel model)
    {
        return ErrorHandling.ExecuteSafely(() =>
        {
            ...
            sb.AppendLine("            .Where(q => q.Where(\"_id\", request._id))");
            ...
            sb.AppendLine($"            .When(request._ids != null && request._ids.AnyItem(), q => q.WhereIn(nameof({model.BaseModel.Name}._id), request._ids))");
            ...
            sb.AppendLine("            {");
            sb.AppendLine($"                _id = context.GetArgument<Guid>(nameof({model.BaseModel.Name}SearchRequest._id)),");
            ...
            sb.AppendLine("            {");
            sb.AppendLine($"                _ids = context.GetArgument<Guid[]?>(nameof({model.BaseModel.Name}sSearchRequest._ids)),");
            sb.AppendLine("                Columns = context.GetSelectColumns(),");
            ...
            foreach (var relation in relationFields)
            {
                ...
                sb.AppendLine("            {");
                sb.AppendLine($"                //{relation.ForeignKeyField} = new Guid[] {{ context.Source._id }},");
                sb.AppendLine("                Columns = context.GetSelectColumns(),");
                sb.AppendLine("                Page = context.HasArgument(\"Page\") ? context.GetArgument<int>(\"Page\") : null,");
                sb.AppendLine("                PageSize = context.HasArgument(\"PageSize\") ? context.GetArgument<int>(\"PageSize\") : null,");
                sb.AppendLine("            };");
