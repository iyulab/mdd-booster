...
/// <para>
/// 속성(attribute) 축은 이 회계 대상이 아니다 — 열린 어휘라 「읽히지 않는 것」의 집합을
/// 열거할 수 없다. enum 헤더의 부모 목록(<c>## X ::enum : Base</c>)도 대상이 아니다: 파서가
/// 부모의 값을 자식의 <c>Values</c> 로 합쳐 주므로(명세 §3.1.6) 모든 타깃이 상속 멤버를 이미
/// 읽고 있다.
/// </para>
/// </summary>
