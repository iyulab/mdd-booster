...
public class ModelAttributeProcessor : IModelProcessor
{
    ...
    private void ProcessFieldAttributes(MDDField field)
    {
        ...
        {
            AppLog.Warning("Skipping field processing - BaseField is null");
            return;
        }

        // Additional null safety for required properties
        if (string.IsNullOrEmpty(field.BaseField.Name) || string.IsNullOrEmpty(field.BaseField.Type))
        {
            AppLog.Warning($"Skipping field processing - Name or Type is null/empty. Name: '{field.BaseField.Name}', Type: '{field.BaseField.Type}'");
            return;
        }
