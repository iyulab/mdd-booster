...
public static class TsFormRenderer
{
    ...
    /// <summary>The first line of every form this renderer emits — how the generator recognises its own files.</summary>
    internal const string Header = "// <auto-generated> mdd-booster; DO NOT EDIT.</auto-generated>";
        ...
        TsFormImports imports)
    {
        ...
        {
            var titleKey = SourceLiteral.TypeScriptString(sectionTitle);
            sb.Append("      <FormSection ").Append(SourceLiteral.JsxStringAttribute("title", sectionTitle))
              .Append(" className={sectionProps?.[").Append(titleKey).Append("]?.className}")
              .Append(" style={sectionProps?.[").Append(titleKey).AppendLine("]?.style}>");
        ...
        IReadOnlySet<string> withExcludedValues)
    {
        ...
        var prop = NameCasing.ToPascalCase(field.Name);
        var label = SourceLiteral.TypeScriptString(MddBooster.Core.Ast.FieldAttributes.EffectiveLabel(field));
        ...
        IReadOnlySet<string> withExcludedValues)
    {
        ...
        var labelAttr = SourceLiteral.JsxStringAttribute("label", label);
        ...
        var descAttr = !string.IsNullOrEmpty(helpText) ? " " + SourceLiteral.JsxStringAttribute("description", helpText) : "";
        ...
        if (control == FormControl.Textarea)
            return $"<UTextarea {labelAttr}{requiredAttr}{descAttr}{disabledAttr}{errorAttr} minRows={{{TextareaMinRows}}} value={{form.{prop} ?? ''}} onChange={{v => onChange({{ {prop}: v || {ClearToken(field)} }})}} />";
        ...
        if (control == FormControl.Select)
        {
            ...
            var placeholder = field.Nullable ? " " + SourceLiteral.JsxStringAttribute("placeholder", $"{label} 선택") : "";
            var onChangeCast = field.Nullable
                ? $"v => onChange({{ {prop}: (v || null) as {enumTypeName} | null }})"
                : $"v => onChange({{ {prop}: v as {enumTypeName} }})";
            return $"<USelect {labelAttr}{requiredAttr}{descAttr}{disabledAttr}{errorAttr}{placeholder} value={{form.{prop} ?? ''}} options={{enumToOptions({labelMap})}} onChange={{{onChangeCast}}} />";
        }
        ...
        if (string.Equals(field.Type, "date", StringComparison.OrdinalIgnoreCase))
        {
            ...
            return $"<UInput {labelAttr}{requiredAttr}{descAttr}{disabledAttr}{errorAttr} type=\"date\" value={{form.{prop} ?? ''}} onChange={{v => onChange({{ {prop}: v || {dateEmpty} }})}} />";
        }
        ...
        if (IsDateTimeType(field.Type))
        {
            ...
            return $"<UInput {labelAttr}{requiredAttr}{descAttr}{disabledAttr}{errorAttr} type=\"datetime\" value={{form.{prop} ?? ''}} onChange={{v => onChange({{ {prop}: v || {dateTimeEmpty} }})}} />";
        }
        ...
        if (IsNumberType(field.Type))
        {
            ...
            return $"<UInput {labelAttr}{requiredAttr}{descAttr}{disabledAttr}{errorAttr} type=\"number\"{stepAttr} value={{form.{prop} != null ? String(form.{prop}) : ''}} onChange={{v => onChange({{ {prop}: v ? Number(v) : {numberEmpty} }})}} />";
        }
        ...
        var maxLenAttr = MddBooster.Core.Ast.FieldAttributes.EffectiveMaxLength(field) is { } n
            ? $" maxlength={{{n}}}"
            : "";
        return $"<UInput {labelAttr}{requiredAttr}{descAttr}{disabledAttr}{errorAttr}{maxLenAttr} value={{form.{prop} ?? ''}} onChange={{v => onChange({{ {prop}: v }})}} />";
    }
